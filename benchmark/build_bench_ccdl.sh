...
function build_with_DAFL() {
    ...
    for TARG in "${@:2}"; do
        ...
        for BUG_NAME in "${str_array[@]:1}"; do
            build_target $1 $CC $CXX "$BIT_OPT -fprofile-instr-generate -fcoverage-mapping" 2>&1 | tee /benchmark/build_log/$BIN_NAME-$BUG_NAME-ccdl.txt
            ### copy results
            copy_build_result $1 $BIN_NAME $BUG_NAME "CCDL"
            # rm -rf RUNDIR-$1
        done
    done
...
# build_with_DAFL "binutils-2.28" "objdump 2017-8396"
