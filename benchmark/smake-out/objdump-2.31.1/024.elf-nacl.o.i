# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 340 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c" 2
# 19 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c"
# 1 "./sysdep.h" 1
# 29 "./sysdep.h"
# 1 "./config.h" 1
# 30 "./sysdep.h" 2
...
# 189 "./sysdep.h" 2
# 20 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c" 2
# 1 "./bfd.h" 1
# 43 "./bfd.h"
...
# 21 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c" 2
# 1 "./libbfd.h" 1
# 58 "./libbfd.h"
...
bfd_boolean _bfd_generic_verify_endian_match
   (bfd *ibfd, struct bfd_link_info *info);
# 22 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c" 2
# 1 "./elf-bfd.h" 1
# 25 "./elf-bfd.h"
...
# 23 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c" 2
# 1 "./elf-nacl.h" 1
# 21 "./elf-nacl.h"
...
# 24 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c" 2
         ...
         bfd_vma minpagesize, bfd_vma sizeof_headers)
{
  ...
  for (i = 0; i < seg->count; ++i)
    {
      if ((seg->sections[i]->flags & (0x10|0x8)) != 0x8)
 return 0;
    }
  return 1;
}
...
{
  const struct elf_backend_data *const bed = ((const struct elf_backend_data *) ((abfd)->xvec)->backend_data);
  struct elf_segment_map **m = &(((abfd) -> tdata.elf_obj_data) -> o->seg_map);
  struct elf_segment_map **first_load = ((void*)0);
  struct elf_segment_map **last_load = ((void*)0);
  bfd_boolean moved_headers = 0;
  int sizeof_headers;

  if (info != ((void*)0) && info->user_phdrs)
  ...
  if (info != ((void*)0))
    ...
    sizeof_headers = ((*((abfd)->xvec->_bfd_sizeof_headers)) (abfd, info));
  else
    {
      ...
      if (seg->p_type == 1)
 {
  ...
  {
# 132 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c"
    struct elf_segment_map *newseg;
    asection *sec;
    struct bfd_elf_section_data *secdata;

    do { if (!(!seg->p_size_valid)) bfd_assert("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c",136); } while (0);
    ...
    secdata->this_hdr.sh_type = 1;
    secdata->this_hdr.sh_flags = (1 << 1) | (1 << 2);
...
{
  struct elf_segment_map **m = &(((abfd) -> tdata.elf_obj_data) -> o->seg_map);
  Elf_Internal_Phdr *phdr = ((abfd) -> tdata.elf_obj_data)->phdr;
  ...
  while (*m != ((void*)0))
    {
      if ((*m)->p_type == 1 && (*m)->includes_filehdr)
 break;
  ...
  if (*m != ((void*)0))
    {
      ...
      struct elf_segment_map **next_load_seg = ((void*)0);
      Elf_Internal_Phdr *next_load_phdr = ((void*)0);
      ...
      while (*m != ((void*)0))
 {
   if (p->p_type == 1 && p->p_vaddr < first_load_phdr->p_vaddr)
     {
       next_load_seg = m;
       next_load_phdr = p;
       break;
...
void
nacl_final_write_processing (bfd *abfd, bfd_boolean linker __attribute__ ((__unused__)))
{
  struct elf_segment_map *seg;
  for (seg = (((abfd) -> tdata.elf_obj_data) -> o->seg_map); seg != ((void*)0); seg = seg->next)
    if (seg->p_type == 1
 && seg->count > 1
 && seg->sections[seg->count - 1]->owner == ((void*)0))
      {
 ...
 do { if (!(sec->flags & 0x100000)) bfd_assert("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c",335); } while (0);
 do { if (!(sec->flags & 0x10)) bfd_assert("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c",336); } while (0);
 do { if (!(sec->size > 0)) bfd_assert("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-nacl.c",337); } while (0);

 fill = abfd->arch_info->fill (sec->size, ((abfd)->xvec->byteorder == BFD_ENDIAN_BIG), 1);

 if (fill == ((void*)0)
     || bfd_seek (abfd, sec->filepos, 0) != 0
     || bfd_bwrite (fill, sec->size, abfd) != sec->size)
   {



     (((abfd) -> tdata.elf_obj_data) -> elf_header)->e_shoff = (file_ptr) -1;
   }
