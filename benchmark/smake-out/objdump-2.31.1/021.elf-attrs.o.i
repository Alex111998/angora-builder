# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 340 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c" 2
# 21 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c"
# 1 "./sysdep.h" 1
# 29 "./sysdep.h"
# 1 "./config.h" 1
# 30 "./sysdep.h" 2
...
# 189 "./sysdep.h" 2
# 22 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c" 2
# 1 "./bfd.h" 1
# 43 "./bfd.h"
...
# 23 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c" 2
...
# 24 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c" 2
# 1 "./libbfd.h" 1
# 58 "./libbfd.h"
...
bfd_boolean _bfd_generic_verify_endian_match
   (bfd *ibfd, struct bfd_link_info *info);
# 25 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c" 2
# 1 "./elf-bfd.h" 1
# 25 "./elf-bfd.h"
...
# 26 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c" 2
...
is_default_attr (obj_attribute *attr)
{
  if (((attr->type) & (1 << 3)))
    return 1;
  if (((attr->type) & (1 << 0)) && attr->i != 0)
    return 0;
  if (((attr->type) & (1 << 1)) && attr->s && *attr->s)
    return 0;
  if (((attr->type) & (1 << 2)))
    return 0;
...
{
  return (vendor == 0
   ? ((const struct elf_backend_data *) ((abfd)->xvec)->backend_data)->obj_attrs_vendor
   : "gnu");
}
...
vendor_obj_attr_size (bfd *abfd, int vendor)
{
  ...
  attr = (((abfd) -> tdata.elf_obj_data) -> known_obj_attributes)[vendor];
  size = 0;
  for (i = 2; i < 71; i++)
    size += obj_attr_size (i, &attr[i]);

  for (list = (((abfd) -> tdata.elf_obj_data) -> other_obj_attributes)[vendor];
       list;
       list = list->next)
    size += obj_attr_size (list->tag, &list->attr);


  return ((size || vendor == 0)
...
bfd_elf_obj_attr_size (bfd *abfd)
{
  ...
  size = vendor_obj_attr_size (abfd, 0);
  size += vendor_obj_attr_size (abfd, 1);
         ...
         int vendor)
{
  ...
  p = contents;
  ((*((abfd)->xvec->bfd_putx32)) ((size),(p)));
  p += 4;
  memcpy (p, vendor_name, vendor_length);
  p += vendor_length;
  *(p++) = Tag_File;
  ((*((abfd)->xvec->bfd_putx32)) ((size - 4 - vendor_length),(p)));
  p += 4;
  ...
  for (i = 2; i < 71; i++)
    {
      unsigned int tag = i;
      if (((const struct elf_backend_data *) ((abfd)->xvec)->backend_data)->obj_attrs_order)
 tag = ((const struct elf_backend_data *) ((abfd)->xvec)->backend_data)->obj_attrs_order (i);
...
bfd_elf_set_obj_attr_contents (bfd *abfd, bfd_byte *contents, bfd_vma size)
{
 ...
 vendor_set_obj_attr_contents (abfd, p, vendor_size, vendor);
  ...
  if (size != my_size)
    _bfd_abort ("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c", 227, __PRETTY_FUNCTION__);
}
...
elf_new_obj_attr (bfd *abfd, int vendor, unsigned int tag)
{
  ...
  if (tag < 71)
    {
      ...
      attr = &(((abfd) -> tdata.elf_obj_data) -> known_obj_attributes)[vendor][tag];
    }
  else
    {
 ...
 bfd_alloc (abfd, sizeof (obj_attribute_list));
      ...
      lastp = &(((abfd) -> tdata.elf_obj_data) -> other_obj_attributes)[vendor];
  ...
  if (tag < 71)
    {
      ...
      return (((abfd) -> tdata.elf_obj_data) -> known_obj_attributes)[vendor][tag].i;
    }
  else
    {
      for (p = (((abfd) -> tdata.elf_obj_data) -> other_obj_attributes)[vendor];
    p;
    p = p->next)
 {
   if (tag == p->tag)
...
_bfd_elf_copy_obj_attributes (bfd *ibfd, bfd *obfd)
{
  ...
  for (vendor = 0; vendor <= 1; vendor++)
    {
      in_attr
 = &(((ibfd) -> tdata.elf_obj_data) -> known_obj_attributes)[vendor][2];
      out_attr
 = &(((obfd) -> tdata.elf_obj_data) -> known_obj_attributes)[vendor][2];
      for (i = 2; i < 71; i++)
 {
   ...
   out_attr++;
      ...
      for (list = (((ibfd) -> tdata.elf_obj_data) -> other_obj_attributes)[vendor];
    list;
    list = list->next)
 {
   in_attr = &list->attr;
   switch (in_attr->type & ((1 << 0) | (1 << 1)))
     {
     case (1 << 0):
     ...
     case (1 << 0) | (1 << 1):
       ...
       break;
     default:
       _bfd_abort ("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c", 391, __PRETTY_FUNCTION__);
     }
 }
    }
}
...
_bfd_elf_obj_attrs_arg_type (bfd *abfd, int vendor, unsigned int tag)
{
  switch (vendor)
    {
    case 0:
      return ((const struct elf_backend_data *) ((abfd)->xvec)->backend_data)->obj_attrs_arg_type (tag);
      break;
    case 1:
      return gnu_obj_attrs_arg_type (tag);
      break;
    default:
      _bfd_abort ("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c", 426, __PRETTY_FUNCTION__);
    }
}
...
_bfd_elf_parse_attributes (bfd *abfd, Elf_Internal_Shdr * hdr)
{
  ...
  if (hdr->sh_size > bfd_get_file_size (abfd))
    {
      ...
      _bfd_error_handler (dgettext ("bfd", "%pB: error: attribute section '%pA' too big: %#llx"),
  ...
  p = contents;
  p_end = p + hdr->sh_size;
  std_sec = ((const struct elf_backend_data *) ((abfd)->xvec)->backend_data)->obj_attrs_vendor;
      ...
      while (len > 0 && p < p_end - 4)
 {
   ...
   section_len = ((*((abfd)->xvec->bfd_getx32)) (p));
   p += 4;
   if (section_len == 0)
     break;
   if (section_len > len)
   ...
   if (section_len <= 4)
     {
       _bfd_error_handler
  (dgettext ("bfd", "%pB: error: attribute section length too small: %" "ll" "d"),
   ...
   while (section_len > 0 && p < p_end)
     {
       ...
       tag = _bfd_safe_read_leb128 (abfd, p, &n, 0, p_end);
       p += n;
       if (p < p_end - 4)
  subsection_len = ((*((abfd)->xvec->bfd_getx32)) (p));
       else
  subsection_len = 0;
       p += 4;
       if (subsection_len == 0)
  ...
  case Tag_File:
    while (p < end)
      {
        ...
        tag = _bfd_safe_read_leb128 (abfd, p, &n, 0, end);
        p += n;
        type = _bfd_elf_obj_attrs_arg_type (abfd, vendor, tag);
        switch (type & ((1 << 0) | (1 << 1)))
   {
   case (1 << 0) | (1 << 1):
     val = _bfd_safe_read_leb128 (abfd, p, &n, 0, end);
     p += n;
   ...
   case (1 << 0):
     ...
     break;
   default:
     _bfd_abort ("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c", 563, __PRETTY_FUNCTION__);
   }
      }
    break;
  case Tag_Section:
...
# 593 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/elf-attrs.c"
bfd_boolean
_bfd_elf_merge_object_attributes (bfd *ibfd, struct bfd_link_info *info)
{
  ...
  for (vendor = 0; vendor <= 1; vendor++)
    {
      ...
      in_attr = &(((ibfd) -> tdata.elf_obj_data) -> known_obj_attributes)[vendor][Tag_compatibility];
      out_attr = &(((obfd) -> tdata.elf_obj_data) -> known_obj_attributes)[vendor][Tag_compatibility];

      if (in_attr->i > 0 && strcmp (in_attr->s, "gnu") != 0)
 {
   _bfd_error_handler

  (dgettext ("bfd", "error: %pB: object has vendor-specific contents that " "must be processed by the '%s' toolchain"),
   ...
   || (in_attr->i != 0 && strcmp (in_attr->s, out_attr->s) != 0))
 {
   ...
   _bfd_error_handler (dgettext ("bfd", "error: %pB: object tag '%d, %s' is " "incompatible with tag '%d, %s'"),
...
_bfd_elf_merge_unknown_attribute_low (bfd *ibfd, bfd *obfd, int tag)
{
  ...
  in_attr = ((((ibfd) -> tdata.elf_obj_data) -> known_obj_attributes) [0]);
  out_attr = ((((obfd) -> tdata.elf_obj_data) -> known_obj_attributes) [0]);

  if (out_attr[tag].i != 0 || out_attr[tag].s != ((void*)0))
    err_bfd = obfd;
  else if (in_attr[tag].i != 0 || in_attr[tag].s != ((void*)0))
    err_bfd = ibfd;

  if (err_bfd != ((void*)0))
    result
      = ((const struct elf_backend_data *) ((err_bfd)->xvec)->backend_data)->obj_attrs_handle_unknown (err_bfd, tag);


  if (in_attr[tag].i != out_attr[tag].i
      || (in_attr[tag].s == ((void*)0)) != (out_attr[tag].s == ((void*)0))
      || (in_attr[tag].s != ((void*)0) && out_attr[tag].s != ((void*)0)
   && strcmp (in_attr[tag].s, out_attr[tag].s) != 0))
    {
      out_attr[tag].i = 0;
      out_attr[tag].s = ((void*)0);
    }
...
_bfd_elf_merge_unknown_attribute_list (bfd *ibfd, bfd *obfd)
{
  ...
  in_list = ((((ibfd) -> tdata.elf_obj_data) -> other_obj_attributes) [0]);
  out_listp = &((((obfd) -> tdata.elf_obj_data) -> other_obj_attributes) [0]);
  out_list = *out_listp;
      ...
      else
 {
   ...
   if (in_list->attr.i != out_list->attr.i
       || (in_list->attr.s == ((void*)0)) != (out_list->attr.s == ((void*)0))
    ...
    && strcmp (in_list->attr.s, out_list->attr.s) != 0))
   ...
   else
     {
      ...
      if (err_bfd)
 result = result
   && ((const struct elf_backend_data *) ((err_bfd)->xvec)->backend_data)->obj_attrs_handle_unknown (err_bfd,
               err_tag);
    }
