# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 340 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c" 2
# 69 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c"
# 1 "./sysdep.h" 1
# 29 "./sysdep.h"
# 1 "./config.h" 1
# 30 "./sysdep.h" 2
...
# 189 "./sysdep.h" 2
# 70 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c" 2
# 1 "./bfd.h" 1
# 43 "./bfd.h"
...
# 71 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c" 2
# 1 "./libbfd.h" 1
# 58 "./libbfd.h"
...
bfd_boolean _bfd_generic_verify_endian_match
   (bfd *ibfd, struct bfd_link_info *info);
# 72 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c" 2
...
# 73 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c" 2
...
# 203 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c"
static void
tekhex_init (void)
{
  unsigned int i;
...
# 238 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c"
...
struct data_struct
{
  unsigned char chunk_data[0x1fff + 1];
  unsigned char chunk_init[(0x1fff + 1 + 32 - 1) / 32];
  bfd_vma vma;
...
getvalue (char **srcp, bfd_vma *valuep, char * endp)
{
  ...
  if (!(((unsigned int) _hex_value[(unsigned char) (*src)]) != 99))
    return 0;

  len = ((unsigned int) _hex_value[(unsigned char) (*src++)]);
  if (len == 0)
    len = 16;
  while (len-- && src < endp)
    {
      if (!(((unsigned int) _hex_value[(unsigned char) (*src)]) != 99))
 return 0;
      value = value << 4 | ((unsigned int) _hex_value[(unsigned char) (*src++)]);
    }
  ...
  if (value != 0)
    {
      ...
      struct data_struct *d = find_chunk (abfd, addr, 1);

      d->chunk_data[addr & 0x1fff] = value;
      d->chunk_init[(addr & 0x1fff) / 32] = 1;
    }
}
...
first_phase (bfd *abfd, int type, char *src, char * src_end)
{
 ...
 while (*src && src < src_end - 1)
   {
     insert_byte (abfd, ((((unsigned int) _hex_value[(unsigned char) ((src)[0])]) << 4) + ((unsigned int) _hex_value[(unsigned char) ((src)[1])])), addr);
     src += 2;
     addr++;
   }
 return 1;
      ...
      while (src < src_end && *src)
 {
  ...
  if (stype <= '4')
    new_symbol->symbol.flags = ((1 << 1) | (1 << 1));
  else
    new_symbol->symbol.flags = (1 << 0);
  if (stype == '2' || stype == '6')
    new_symbol->symbol.section = (&_bfd_std_section[2]);
  else if (stype == '3' || stype == '7')
    {
      if ((section->flags & 0x20) == 0)
        section->flags |= 0x10;
      else
        {
   if (alt_section == ((void*)0))
     alt_section
       = bfd_get_next_section_by_name (((void*)0), section);
   if (alt_section == ((void*)0))
     alt_section = bfd_make_section_anyway_with_flags
       (abfd, section->name,
        (section->flags & ~0x20) | 0x10);
   if (alt_section == ((void*)0))
     return 0;
...
pass_over (bfd *abfd, bfd_boolean (*func) (bfd *, int, char *, char *))
{
 ...
 return 0;
      ...
      if (!(((unsigned int) _hex_value[(unsigned char) (src[0])]) != 99) || !(((unsigned int) _hex_value[(unsigned char) (src[1])]) != 99))
 break;


      chars_on_line = ((((unsigned int) _hex_value[(unsigned char) ((src)[0])]) << 4) + ((unsigned int) _hex_value[(unsigned char) ((src)[1])])) - 5;

      if (chars_on_line >= 0xff)
 return 0;
...
tekhex_object_p (bfd *abfd)
{
  ...
  if (b[0] != '%' || !(((unsigned int) _hex_value[(unsigned char) (b[1])]) != 99) || !(((unsigned int) _hex_value[(unsigned char) (b[2])]) != 99) || !(((unsigned int) _hex_value[(unsigned char) (b[3])]) != 99))
    return ((void*)0);
         ...
         bfd_boolean get)
{
  ...
  do { if (!(offset == 0)) bfd_assert("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c",638); } while (0);
  for (addr = section->vma; count != 0; count--, addr++)
    {

      bfd_vma chunk_number = addr & ~(bfd_vma) 0x1fff;
      bfd_vma low_bits = addr & 0x1fff;
...
tekhex_get_section_contents (bfd *abfd,
        ...
        bfd_size_type count)
{
  if (section->flags & (0x2 | 0x1))
    {
      move_section_contents (abfd, section, locationp, offset, count, 1);
      return 1;
    }
...
out (bfd *abfd, int type, char *start, char *end)
{
  ...
  front[0] = '%';
  (front + 1)[1] = digs[(end - start + 5) & 0xf]; (front + 1)[0] = digs[((end - start + 5)>>4)&0xf];;
  front[3] = type;
  ...
  (front + 4)[1] = digs[(sum) & 0xf]; (front + 4)[0] = digs[((sum)>>4)&0xf];;
  if (bfd_bwrite (front, (bfd_size_type) 6, abfd) != 6)
    _bfd_abort ("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c", 792, __PRETTY_FUNCTION__);
  end[0] = '\n';
  wrlen = end - start + 1;
  if (bfd_bwrite (start, wrlen, abfd) != wrlen)
    _bfd_abort ("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c", 796, __PRETTY_FUNCTION__);
}
...
tekhex_write_object_contents (bfd *abfd)
{
  ...
  for (d = abfd->tdata.tekhex_data->data;
       ...
       d = d->next)
    {
      ...
      for (addr = 0; addr < 0x1fff + 1; addr += 32)
 {
   if (d->chunk_init[addr / 32])
     {
       char *dst = buffer;
  ...
  {
    (dst)[1] = digs[(d->chunk_data[addr + low]) & 0xf]; (dst)[0] = digs[((d->chunk_data[addr + low])>>4)&0xf];;
    dst += 2;
  }
      ...
      for (p = abfd->outsymbols; *p; p++)
 {
  ...
  if (bfd_bwrite ("%0781010\n", (bfd_size_type) 9, abfd) != 9)
    _bfd_abort ("/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c", 901, __PRETTY_FUNCTION__);
  return 1;
}

static int
tekhex_sizeof_headers (bfd *abfd __attribute__ ((__unused__)),
...
tekhex_make_empty_symbol (bfd *abfd)
{
  ...
  if (!new_symbol)
    return ((void*)0);
  new_symbol->symbol.the_bfd = abfd;
  new_symbol->prev = ((void*)0);
  return &(new_symbol->symbol);
}

static void
tekhex_get_symbol_info (bfd *abfd __attribute__ ((__unused__)),
   asymbol *symbol,
   symbol_info *ret)
{
...
# 995 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/bfd/tekhex.c"
const bfd_target tekhex_vec =
{
  ...
  _bfd_archive_close_and_cleanup, _bfd_bool_bfd_true, _bfd_generic_new_section_hook, tekhex_get_section_contents, _bfd_generic_get_section_contents_in_window,
  ...
  tekhex_get_symtab_upper_bound, tekhex_canonicalize_symtab, tekhex_make_empty_symbol, tekhex_print_symbol, tekhex_get_symbol_info, _bfd_nosymbols_get_symbol_version_string, bfd_generic_is_local_label_name, _bfd_bool_bfd_asymbol_false, _bfd_nosymbols_get_lineno, _bfd_nosymbols_find_nearest_line, _bfd_nosymbols_find_line, _bfd_nosymbols_find_inliner_info, _bfd_nosymbols_bfd_make_debug_symbol, _bfd_generic_read_minisymbols, _bfd_generic_minisymbol_to_symbol,
