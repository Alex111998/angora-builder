# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 340 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
# 24 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c"
# 1 "./sysdep.h" 1
# 23 "./sysdep.h"
...
# 180 "./sysdep.h" 2
# 25 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
# 1 "../bfd/bfd.h" 1
# 43 "../bfd/bfd.h"
...
# 26 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
...
# 27 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
...
# 28 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
...
# 30 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 31 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
# 1 "./bucomm.h" 1
# 27 "./bucomm.h"
...
# 32 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c" 2
# 41 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c"
char *program_name;
...
# 68 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c"
void
        ...
        const char *format, ...)
{
 ...
 filename = bfd_get_archive_filename (abfd);
      if (section)
 section_name = ((void) abfd, (section)->name);
    }
  if (section_name)
...
set_default_bfd_target (void)
{
  ...
  if (! bfd_set_default_target (target))
    fatal (gettext ("can't set BFD default target to `%s': %s"),
...
{
  fflush (stdout);
  fprintf (stderr, gettext ("%s: Matching formats:"), program_name);
  while (*p)
    fprintf (stderr, " %s", *p++);
  fputc ('\n', stderr);
}
...
list_supported_targets (const char *name, FILE *f)
{
  ...
  if (name == ((void*)0))
    fprintf (f, gettext ("Supported targets:"));
  else
    fprintf (f, gettext ("%s: supported targets:"), name);

  targ_names = bfd_target_list ();
  for (t = 0; targ_names[t] != ((void*)0); t++)
...
list_supported_architectures (const char *name, FILE *f)
{
  ...
  if (name == ((void*)0))
    fprintf (f, gettext ("Supported architectures:"));
  else
    fprintf (f, gettext ("%s: supported architectures:"), name);
...
endian_string (enum bfd_endian endian)
{
  switch (endian)
    {
    case BFD_ENDIAN_BIG: return gettext ("big endian");
    case BFD_ENDIAN_LITTLE: return gettext ("little endian");
    default: return gettext ("endianness unknown");
    }
}
...
do_display_target (const bfd_target *targ, void *data)
{
 ...
 if (((*((abfd)->xvec->_bfd_set_arch_mach)) (abfd, a, 0)))
   {
...
{
  arg->filename = make_temp_file (((void*)0));
  arg->error = 0;
  arg->count = 0;
  arg->alloc = 0;
  arg->info = ((void*)0);
...
display_info (void)
{
  ...
  printf (gettext ("BFD header file version %s\n"), "(GNU Binutils) " "2.31.1");
...
print_arelt_descr (FILE *file, bfd *abfd, bfd_boolean verbose, bfd_boolean offsets)
{
  ...
  if (verbose)
    {
      if (((*((abfd)->xvec->_bfd_stat_arch_elt)) (abfd, &buf)) == 0)
 {
   char modebuf[11];
   char timebuf[40];
   time_t when = buf.st_mtim.tv_sec;
   const char *ctime_result = (const char *) ctime (&when);
   bfd_size_type size;


   if (ctime_result == ((void*)0))
     sprintf (timebuf, gettext ("<time data corrupt>"));
   else
   ...
   fprintf (file, "%s %ld/%ld %6" "l" "u %s ", modebuf + 1,
     (long) buf.st_uid, (long) buf.st_gid,
     size, timebuf);
 }
    }

  fprintf (file, "%s", ((char *) (abfd)->filename));

  if (offsets)
    {
      if (((abfd)->is_thin_archive) && abfd->proxy_origin)
        fprintf (file, " 0x%lx", (unsigned long) abfd->proxy_origin);
      else if (!((abfd)->is_thin_archive) && abfd->origin)
...
template_in_dir (const char *path)
{
  ...
  char *tmpname;
  size_t len;
# 497 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c"
  if (slash != (char *) ((void*)0))
    {
      len = slash - path;
      tmpname = (char *) xmalloc (len + sizeof ("stXXXXXX") + 2);
      memcpy (tmpname, path, len);
# 510 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c"
      tmpname[len++] = '/';
    }
  else
    {
      tmpname = (char *) xmalloc (sizeof ("stXXXXXX"));
      len = 0;
    }
...
# 572 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c"
}
...
parse_vma (const char *s, const char *arg)
{
  ...
  if (*end != '\0')
    fatal (gettext ("%s: bad number: %s"), arg, s);
...
get_file_size (const char * file_name)
{
 ...
 non_fatal (gettext ("'%s': No such file"), file_name);
      else
 non_fatal (gettext ("Warning: could not locate '%s'.  reason: %s"),
     file_name, strerror ((*__errno_location ())));
    }
  else if (((((statbuf.st_mode)) & 0170000) == (0040000)))
    non_fatal (gettext ("Warning: '%s' is a directory"), file_name);
  else if (! ((((statbuf.st_mode)) & 0170000) == (0100000)))
    non_fatal (gettext ("Warning: '%s' is not an ordinary file"), file_name);
  else if (statbuf.st_size < 0)
    non_fatal (gettext ("Warning: '%s' has negative size, probably it is too large"),
               file_name);
  else
...
bfd_get_archive_filename (const bfd *abfd)
{
  ...
  ((void) sizeof ((abfd != ((void*)0)) ? 1 : 0), __extension__ ({ if (abfd != ((void*)0)) ; else __assert_fail ("abfd != NULL", "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/bucomm.c", 633, __extension__ __PRETTY_FUNCTION__); }));

  if (abfd->my_archive == ((void*)0)
      || ((abfd->my_archive)->is_thin_archive))
    return ((char *) (abfd)->filename);

  needed = (strlen (((char *) (abfd->my_archive)->filename))
     + strlen (((char *) (abfd)->filename)) + 3);
  if (needed > curr)
    {
      if (curr)
 free (buf);
  ...
  sprintf (buf, "%s(%s)", ((char *) (abfd->my_archive)->filename),
    ((char *) (abfd)->filename));
  return buf;
}
