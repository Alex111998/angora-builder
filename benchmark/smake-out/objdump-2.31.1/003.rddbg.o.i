# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 340 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c" 2
# 27 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c"
# 1 "./sysdep.h" 1
# 23 "./sysdep.h"
...
# 180 "./sysdep.h" 2
# 28 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c" 2
# 1 "../bfd/bfd.h" 1
# 43 "../bfd/bfd.h"
...
# 29 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c" 2
...
# 30 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c" 2
# 1 "./bucomm.h" 1
# 27 "./bucomm.h"
...
# 31 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c" 2
# 1 "./debug.h" 1
# 33 "./debug.h"
...
# 32 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c" 2
# 1 "./budbg.h" 1
# 27 "./budbg.h"
...
# 33 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/rddbg.c" 2
...
read_debugging_info (bfd *abfd, asymbol **syms, long symcount, bfd_boolean no_messages)
{
 ...
 return ((void*)0);
  ...
  if (! found)
    {
      if (! no_messages)
 non_fatal (gettext ("%s: no recognized debugging information"),
     ((char *) (abfd)->filename));
      return ((void*)0);
    }
       ...
       void *dhandle, bfd_boolean *pfound)
{
      ...
      if (sec != ((void*)0) && strsec != ((void*)0))
 {
   ...
   if (! bfd_get_section_contents (abfd, sec, stabs, 0, stabsize))
     {
       fprintf (stderr, "%s: %s: %s\n",
         ((char *) (abfd)->filename), names[i].secname,
   ...
   if (! bfd_get_section_contents (abfd, strsec, strings, 0, strsize))
     {
       fprintf (stderr, "%s: %s: %s\n",
         ((char *) (abfd)->filename), names[i].strsecname,
         bfd_errmsg (bfd_get_error ()));
       return 0;
     }
   ...
   if (shandle == ((void*)0))
     {
       shandle = start_stab (dhandle, abfd, 1, syms, symcount);
       if (shandle == ((void*)0))
  return 0;
     }
   ...
   for (stab = stabs; stab <= (stabs + stabsize) - 12; stab += 12)
     {
       unsigned int strx;
       int type;
       int other __attribute__ ((__unused__));
       int desc;
       bfd_vma value;



       strx = ((*((abfd)->xvec->bfd_getx32)) (stab));
       type = (*(const unsigned char *) (stab + 4) & 0xff);
       other = (*(const unsigned char *) (stab + 5) & 0xff);
       desc = ((*((abfd)->xvec->bfd_getx16)) (stab + 6));
       value = ((*((abfd)->xvec->bfd_getx32)) (stab + 8));
       ...
       else
  {
    ...
    if (stroff + strx >= strsize)
      {
        fprintf (stderr, gettext ("%s: %s: stab entry %ld is corrupt, strx = 0x%x, type = %d\n"),
          ((char *) (abfd)->filename), names[i].secname,
    ...
    && stab + 12 < stabs + stabsize)
      {
        ...
        p = s + len - 1;
        *p = '\0';
        strx = stroff + ((*((abfd)->xvec->bfd_getx32)) (stab));
        if (strx >= strsize)
   {
     fprintf (stderr, gettext ("%s: %s: stab entry %ld is corrupt\n"),
       ((char *) (abfd)->filename), names[i].secname,
       (long) (stab - stabs) / 12);
     break;
   }
      ...
      void *dhandle, bfd_boolean *pfound)
{
  ...
  for (ps = syms; ps < symend; ps++)
    {
      ...
      ((*((abfd)->xvec->_bfd_get_symbol_info)) (abfd,*ps,&i));
      ...
      if (i.type == '-')
 {
   ...
   if (shandle == ((void*)0))
     {
       shandle = start_stab (dhandle, abfd, 0, syms, symcount);
       if (shandle == ((void*)0))
  return 0;
     }
   ...
   && ps + 1 < symend)
     {
       ...
       sc = xstrdup (s);
       sc[strlen (sc) - 1] = '\0';
       n = concat (sc, ((*ps)->name), (const char *) ((void*)0));
       free (sc);
       if (f != ((void*)0))
  free (f);
       f = n;
...
static struct saved_stab saved_stabs[(16)];
...
save_stab (int type, int desc, bfd_vma value, const char *string)
{
  ...
  saved_stabs_index = (saved_stabs_index + 1) % (16);
}
...
stab_context (void)
{
  ...
  fprintf (stderr, gettext ("Last stabs entries before error:\n"));
      ...
      if (stabp->string != ((void*)0))
 {
   ...
   fprintf (stderr, "%08" "l" "x", stabp->value);
   if (stabp->type != 0)
