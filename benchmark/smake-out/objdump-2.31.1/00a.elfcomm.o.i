# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 340 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
# 24 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c"
# 1 "./sysdep.h" 1
# 23 "./sysdep.h"
...
# 180 "./sysdep.h" 2
# 25 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
...
# 26 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
...
# 27 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
# 1 "../bfd/bfd.h" 1
# 43 "../bfd/bfd.h"
...
# 28 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
...
# 29 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
# 1 "./bucomm.h" 1
# 27 "./bucomm.h"
...
# 30 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
# 1 "./elfcomm.h" 1
# 29 "./elfcomm.h"
...
extern char *make_qualified_name (struct archive_info *,
      struct archive_info *,
      const char *);
# 31 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 32 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c" 2
...
error (const char *message, ...)
{
  ...
  fprintf (stderr, gettext ("%s: Error: "), program_name);
...
warn (const char *message, ...)
{
  ...
  __builtin_va_start(args, message);
  fprintf (stderr, gettext ("%s: Warning: "), program_name);
  vfprintf (stderr, message, args);
  __builtin_va_end(args);
}
...
byte_put_little_endian (unsigned char * field, elf_vma value, int size)
{
  switch (size)
    ...
    default:
      error (gettext ("Unhandled data length: %d\n"), size);
      abort ();
    }
}
        ...
        unsigned long name_len)
{
  ...
  if ((((((name)[0]) == '/') || ((((name)[0]) == '\\') && (0))) || ((name)[0] && ((name)[1] == ':') && (0))) || base_name == file_name)
    {
      amt = name_len + 1;
      if (amt == 0)
 return ((void*)0);
 ...
 {
   error (gettext ("Abnormal length of thin archive member name: %lx\n"),
   name_len);
   return ((void*)0);
 }
...
# 462 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c"
static bfd_boolean
       ...
       bfd_boolean read_symbols)
{
  ...
  size = strtoul (arch->arhdr.ar_size, ((void*)0), 10);
  arch->arhdr.ar_fmag[0] = fmag_save;

  if ((signed long) size < 0)
    {
      error (gettext ("%s: invalid archive header size: %ld\n"),
      arch->file_name, size);
      return 0;
    }
  ...
  if (! read_symbols)
    {
      if (fseek (arch->file, size, 1) != 0)
 {
   error (gettext ("%s: failed to skip archive symbol table\n"),
   arch->file_name);
   return 0;
 }
    }
  else
    {
      ...
      ((void) sizeof ((sizeof_ar_index <= sizeof integer_buffer) ? 1 : 0), __extension__ ({ if (sizeof_ar_index <= sizeof integer_buffer) ; else __assert_fail ("sizeof_ar_index <= sizeof integer_buffer", "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c", 505, __extension__ __PRETTY_FUNCTION__); }));


      if (size < sizeof_ar_index)
 {
   error (gettext ("%s: the archive index is empty\n"), arch->file_name);
   return 0;
 }
 ...
 {
   error (gettext ("%s: failed to read archive index\n"), arch->file_name);
   return 0;
 }
   ...
   || size < arch->index_num)
 {
   error (gettext ("%s: the archive index is supposed to have 0x%lx entries of %d bytes, but the size is only 0x%lx\n"),
 ...
 malloc (arch->index_num * sizeof_ar_index);
      if (index_buffer == ((void*)0))
 {
   error (gettext ("Out of memory whilst trying to read archive symbol index\n"));
   return 0;
 }
 ...
 malloc (arch->index_num * sizeof (* arch->index_array));
      if (arch->index_array == ((void*)0))
 {
   free (index_buffer);
   error (gettext ("Out of memory whilst trying to convert the archive symbol index\n"));
   return 0;
 }
 ...
 arch->index_array[i] =
   byte_get_big_endian ((unsigned char *) (index_buffer + (i * sizeof_ar_index)),
      ...
      if (size < 1)
 {
   error (gettext ("%s: the archive has an index but no symbols\n"),
   arch->file_name);
   return 0;
 }
 ...
 {
   error (gettext ("Out of memory whilst trying to read archive index symbol table\n"));
   return 0;
 }
      ...
      if (got != size)
 {
   error (gettext ("%s: failed to read archive index symbol table\n"),
   arch->file_name);
   return 0;
 }
    }
  ...
  if (got != sizeof arch->arhdr && got != 0)
    {
      error (gettext ("%s: failed to read archive header following archive index\n"),
      arch->file_name);
      return 0;
    }
        ...
        bfd_boolean read_symbols)
{
  ...
  arch->next_arhdr_offset = 8;


  if (fseek (file, 8, 0) != 0)
    {
      error (gettext ("%s: failed to seek to first archive header\n"), file_name);
      return 1;
    }
 ...
 return 0;
  ...
  if ((strncmp ((arch->arhdr.ar_name), ("/               "), sizeof ("/               ") - 1) == 0))
    {
      if (! process_archive_index_and_symbols (arch, 4, read_symbols))
 return 1;
    }
  else if ((strncmp ((arch->arhdr.ar_name), ("/SYM64/         "), sizeof ("/SYM64/         ") - 1) == 0))
    {
 ...
 return 1;
    ...
    printf (gettext ("%s has no archive index\n"), file_name);
  ...
  if ((strncmp ((arch->arhdr.ar_name), ("//              "), sizeof ("//              ") - 1) == 0))
    {
      ...
      arch->arhdr.ar_fmag[0] = 0;
      arch->longnames_size = strtoul (arch->arhdr.ar_size, ((void*)0), 10);
      arch->arhdr.ar_fmag[0] = fmag_save;

      if (arch->longnames_size < 8)
 {
   error (gettext ("%s: long name table is too small, (size = %ld)\n"),
 ...
 {
   error (gettext ("%s: long name table is too big, (size = 0x%lx)\n"),
   file_name, arch->longnames_size);
   return 1;
 }
      ...
      if (arch->longnames == ((void*)0))
 {
   error (gettext ("Out of memory reading long symbol names in archive\n"));
   return 1;
 }
      ...
      if (fread (arch->longnames, arch->longnames_size, 1, file) != 1)
 {
   free (arch->longnames);
   arch->longnames = ((void*)0);
   error (gettext ("%s: failed to read long symbol name string table\n"),
   file_name);
   return 1;
 }
        ...
        const char *member_file_name)
{
  ...
  if (nested_arch->file_name != ((void*)0)
      && (strcmp ((nested_arch->file_name), (member_file_name)) == 0))
    return 0;
...
# 756 "/benchmark/RUNDIR-binutils-2.31.1/binutils-2.31.1/binutils/elfcomm.c"
char *
                         ...
                         struct archive_info *nested_arch)
{
  ...
  if (arch->arhdr.ar_name[0] == '/')
    {
      ...
      if (arch->longnames == ((void*)0) || arch->longnames_size == 0)
 {
   error (gettext ("Archive member uses long names, but no longname table found\n"));
   return ((void*)0);
 }
      ...
      if (arch->is_thin_archive && endp != ((void*)0) && * endp == ':')
        arch->nested_member_origin = strtoul (endp + 1, ((void*)0), 10);
      arch->arhdr.ar_fmag[0] = fmag_save;

      if (j > arch->longnames_size)
 {
   error (gettext ("Found long name index (%ld) beyond end of long name table\n"),j);
   return ((void*)0);
 }
 ...
 j = arch->longnames_size;
      ...
      if (k >= j)
 {
   error (gettext ("Invalid Thin archive member name\n"));
   return ((void*)0);
 }
       ...
       struct archive_info *nested_arch)
{
  ...
  if (fseek (arch->file, offset, 0) != 0)
    {
      error (gettext ("%s: failed to seek to next file name\n"), arch->file_name);
      return ((void*)0);
    }
  got = fread (&arch->arhdr, 1, sizeof arch->arhdr, arch->file);
  if (got != sizeof arch->arhdr)
...
{
  const char * error_name = gettext ("<corrupt>");
  size_t len;
  char * name;
