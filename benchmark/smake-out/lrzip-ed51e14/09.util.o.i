# 1 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 343 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
# 35 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c"
# 1 "./config.h" 1
# 36 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 39 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 41 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 43 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 51 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 52 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 53 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 54 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
# 1 "./lrzip_private.h" 1
# 23 "./lrzip_private.h"
# 1 "./config.h" 1
...
# 55 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
# 1 "./util.h" 1
# 23 "./util.h"
...
# 56 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
# 1 "./sha4.h" 1
# 40 "./sha4.h"
typedef struct
{
...
# 57 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
# 1 "./aes.h" 1
# 38 "./aes.h"
typedef struct
{
...
# 58 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
# 60 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c" 2
...
void setup_ram(rzip_control *control)
{
 ...
 if ((control->flags & (1 << 13)) && (((control->flags & (1 << 12)) && (control->flags & (1 << 4))) || !((control->flags & (1 << 4)) || (control->flags & (1 << 2)))))
 ...
 if ((sizeof(long) == 4)) {
  ...
  control->usable_ram = ((control->ramsize - 900000000ll) > (900000000ll)? (control->ramsize - 900000000ll): (900000000ll));
  control->maxram = ((control->maxram) < (control->usable_ram)? (control->maxram): (control->usable_ram));
  control->maxram = ((control->maxram) < ((1000 * 1024 * 1024) * 2 / 3)? (control->maxram): ((1000 * 1024 * 1024) * 2 / 3));
 } else
...
_Bool get_rand(rzip_control *control, unsigned char *buf, int len)
{
 ...
 if (fd == -1) {
  ...
  if (__builtin_expect(!!(read(fd, buf, len) != len), 0))
   do { fatal(control, 174, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Failed to read fd in get_rand\n"); return 0; } while (0);
  if (__builtin_expect(!!(close(fd)), 0))
   do { fatal(control, 176, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Failed to close fd in get_rand\n"); return 0; } while (0);
 }
 return 1;
}
...
_Bool read_config(rzip_control *control)
{
 ...
 while ((fgets(line, 255, fp)) != ((void*)0)) {
  ...
  if (((*__ctype_b_loc ())[(int) ((*parameter))] & (unsigned short int) _ISspace))
   continue;
  if (*parameter == '#')
   continue;

  parametervalue = strtok(((void*)0), " =");
  ...
  if ((!strcasecmp( parameter, "window" )))
   ...
   if ( control->compression_level < 1 || control->compression_level > 9 )
    do { failure(control, 238, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "CONF.FILE error. Compression Level must between 1 and 9"); return 0; } while (0);
  } else if ((!strcasecmp( parameter, "compressionmethod" ))) {

   if (control->flags & ((1 << 5) | (1 << 6) | (1 << 7) | (1 << 8) | (1 << 9)))
    do { failure(control, 242, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "CONF.FILE error. Can only specify one compression method"); return 0; } while (0);
   if ((!strcasecmp( parametervalue, "bzip2" )))
    ...
    do { failure(control, 254, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "CONF.FILE error. Invalid compression method %s specified\n",parametervalue); return 0; } while (0);
  } else if ((!strcasecmp( parameter, "lzotest" ))) {

   if ((!strcasecmp( parametervalue, "no" )))
    control->flags &= ~(1 << 20);
  } else if ((!strcasecmp( parameter, "hashcheck" ))) {
   if ((!strcasecmp( parametervalue, "yes" ))) {
    control->flags |= (1 << 18);
    control->flags |= (1 << 16);
   }
  } else if ((!strcasecmp( parameter, "showhash" ))) {
   if ((!strcasecmp( parametervalue, "yes" )))
    control->flags |= (1 << 16);
  } else if ((!strcasecmp( parameter, "outputdirectory" ))) {
   control->outdir = malloc(strlen(parametervalue) + 2);
   if (!control->outdir)
    do { fatal(control, 270, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Fatal Memory Error in read_config"); return 0; } while (0);
   ...
   if (control->flags & ((1 << 10) | (1 << 11)))
    do { failure(control, 276, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "CONF.FILE error. Verbosity already defined."); return 0; } while (0);
   if ((!strcasecmp( parametervalue, "yes" )))
    control->flags |= (1 << 10);
   else if ((!strcasecmp( parametervalue, "max" )))
    control->flags |= (1 << 11);
   else
    do { print_err(control, 282, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "lrzip.conf: Unrecognized verbosity value %s. Ignored.\n", parametervalue); } while (0);
   ...
   if (control->nice_val < -20 || control->nice_val > 19)
    do { failure(control, 290, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "CONF.FILE error. Nice must be between -20 and 19"); return 0; } while (0);
  } else if ((!strcasecmp( parameter, "keepbroken" ))) {
   if ((!strcasecmp( parametervalue, "yes" )))
    control->flags |= (1 << 19);
   ...
   control->tmpdir = realloc(((void*)0), strlen(parametervalue) + 2);
   if (!control->tmpdir)
    do { fatal(control, 305, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Fatal Memory Error in read_config"); return 0; } while (0);
   strcpy(control->tmpdir, parametervalue);
   if (strcmp(parametervalue + strlen(parametervalue) - 1, "/"))
    strcat(control->tmpdir, "/");
  } else if ((!strcasecmp( parameter, "encrypt" ))) {
   if ((!strcasecmp( parameter, "YES" )))
    control->flags |= (1 << 23);
  } else

   do { print_err(control, 315, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "lrzip.conf: Unrecognized parameter value, %s = %s. Continuing.\n", parameter, parametervalue); } while (0);

 }

 if (__builtin_expect(!!(fclose(fp)), 0))
  do { fatal(control, 319, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Failed to fclose fp in read_config\n"); return 0; } while (0);
...
static void lrz_keygen(const rzip_control *control, const unsigned char *salt, unsigned char *key, unsigned char *iv)
{
 unsigned char buf [64 + 8 + 512];
 mlock(buf, 64 + 8 + 512);

 memcpy(buf, control->hash, 64);
 memcpy(buf + 64, salt, 8);
 memcpy(buf + 64 + 8, control->salt_pass, control->salt_pass_len);
 sha4(buf, 64 + 8 + control->salt_pass_len, key, 0);

 memcpy(buf, key, 64);
 memcpy(buf + 64, salt, 8);
 memcpy(buf + 64 + 8, control->salt_pass, control->salt_pass_len);
 sha4(buf, 64 + 8 + control->salt_pass_len, iv, 0);
...
_Bool lrz_crypt(const rzip_control *control, unsigned char *buf, i64 len, const unsigned char *salt, int encrypt)
{
 ...
 if (encrypt == (1)) {
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 379, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Encrypting data        \n"); } while (0); } while (0);
  if (__builtin_expect(!!(aes_setkey_enc(&aes_ctx, key, 128)), 0))
   do { failure(control, 381, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Failed to aes_setkey_enc in lrz_crypt\n"); goto error; } while (0);
  aes_crypt_cbc(&aes_ctx, 1, N, iv, buf, buf);

  if (M) {
   memset(tmp0, 0, 16);
   memcpy(tmp0, buf + N, M);
   aes_crypt_cbc(&aes_ctx, 1, 16,
    iv, tmp0, tmp1);
   memcpy(buf + N, buf + N - 16, M);
   memcpy(buf + N - 16, tmp1, 16);
  }
 } else {
  if (__builtin_expect(!!(aes_setkey_dec(&aes_ctx, key, 128)), 0))
   do { failure(control, 394, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Failed to aes_setkey_dec in lrz_crypt\n"); goto error; } while (0);
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 395, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Decrypting data        \n"); } while (0); } while (0);
  if (M) {
   aes_crypt_cbc(&aes_ctx, 0, N - 16,
          iv, buf, buf);
   aes_crypt_ecb(&aes_ctx, 0,
          buf + N - 16, tmp0);
   memset(tmp1, 0, 16);
   memcpy(tmp1, buf + N, M);
   xor128(tmp0, tmp1);
   memcpy(buf + N, tmp0, M);
   memcpy(tmp1 + M, tmp0 + M, 16 - M);
   aes_crypt_ecb(&aes_ctx, 0, tmp1,
          buf + N - 16);
   xor128(buf + N - 16, iv);
  } else
   aes_crypt_cbc(&aes_ctx, 0, len,
          iv, buf, buf);
 }
...
void lrz_stretch(rzip_control *control)
{
 ...
 n = control->encloops * 64 / (control->salt_pass_len + sizeof(i64));
 do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 434, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/util.c", __func__, "Hashing passphrase %lld (%lld) times \n", control->encloops, n); } while (0); } while (0);
 for (j = 0; j < n; j ++) {
  counter = __uint64_identity (j);
  sha4_update(&ctx, (unsigned char *)&counter, sizeof(counter));
