# 1 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 343 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
# 21 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c"
# 1 "./config.h" 1
# 22 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 25 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 27 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 29 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
extern int fstatvfs64 (int __fildes, struct statvfs64 *__buf)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (2)));
# 30 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 32 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 34 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 36 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 50 "/usr/include/x86_64-linux-gnu/bits/mman-shared.h" 3 4
...
# 38 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 39 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 40 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 49 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
extern int utime (const char *__file,
    const struct utimbuf *__file_times)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (1)));
# 50 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 51 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
# 56 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
# 1 "./stream.h" 1
# 26 "./stream.h"
_Bool create_pthread(rzip_control *control, pthread_t *thread, pthread_attr_t * attr,
 void * (*start_routine)(void *), void *arg);
_Bool join_pthread(pthread_t th, void **thread_return);
...
# 57 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c" 2
...
static i64 fdout_seekto(rzip_control *control, i64 pos)
{
 if ((control->flags & (1 << 21))) {
  ...
  control->out_ofs = pos;
  if (__builtin_expect(!!(pos > control->out_len || pos < 0), 0)) {
   do { print_err(control, 68, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Trying to seek to %lld outside tmp outbuf in fdout_seekto\n", pos); } while (0);
   return -1;
  }
  return 0;
 }
 return lseek(control->fd_out, pos, 0);
}
# 94 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c"
i64 get_ram(rzip_control *control)
{
 ...
 ramsize = (i64)sysconf(_SC_PHYS_PAGES) * (sysconf(_SC_PAGESIZE));
 if (ramsize > 0)
  return ramsize;


 if(!(meminfo = fopen("/proc/meminfo", "r")))
  do { fatal(control, 106, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "fopen\n"); return -1; } while (0);

 while(!feof(meminfo) && !fscanf(meminfo, "MemTotal: %""l" "d"" kB", &ramsize)) {
  if (__builtin_expect(!!(fgets(aux, sizeof(aux), meminfo) == ((void*)0)), 0)) {
   fclose(meminfo);
   do { fatal(control, 111, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to fgets in get_ram\n"); return -1; } while (0);
  }
 }
 if (fclose(meminfo) == -1)
  do { fatal(control, 115, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "fclose"); return -1; } while (0);
 ramsize *= 1000;
...
i64 nloops(i64 seconds, unsigned char *b1, unsigned char *b2)
{
 ...
 nloops = (1000000 * pow (pow (1.835, 1.0 / (365*86400)), -1293840000)) * pow(pow (1.835, 1.0 / (365*86400)), seconds);
 if (nloops < 1000000)
  nloops = 1000000;
...
_Bool write_magic(rzip_control *control)
{
 ...
 else if (!(control->flags & (1 << 12)) || !(control->flags & (1 << 13)) || control->eof) {
  i64 esize = __uint64_identity (control->st_size);
 ...
 if (!(!((control->flags & (1 << 16))) && !((control->flags & (1 << 17)))))
  magic[21] = 1;
 if ((control->flags & (1 << 23)))
  magic[22] = 1;

 if (__builtin_expect(!!(fdout_seekto(control, 0)), 0))
  do { fatal(control, 173, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to seek to BOF to write Magic Header\n"); return 0; } while (0);

 if (__builtin_expect(!!(put_fdout(control, magic, (24)) != (24)), 0))
  do { fatal(control, 176, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to write magic header\n"); return 0; } while (0);
 control->magic_written = 1;
 return 1;
}

static inline i64 enc_loops(unsigned char b1, unsigned char b2)
{
 return (i64)b2 << (i64)b1;
}

static _Bool get_magic(rzip_control *control, char *magic)
{
 int encrypted, md5, i;
 i64 expected_size;
 uint32_t v;

 if (__builtin_expect(!!(strncmp(magic, "LRZI", 4)), 0))
  do { failure(control, 193, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Not an lrzip file\n"); return 0; } while (0);

 memcpy(&control->major_version, &magic[4], 1);
 memcpy(&control->minor_version, &magic[5], 1);

 do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 198, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Detected lrzip version %d.%d file.\n", control->major_version, control->minor_version); } while (0); } while (0);
 if (control->major_version > 0 ||
     (control->major_version == 0 && control->minor_version > 6))
  do { do { print_stuff(control, 1, 201, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Attempting to work with file produced by newer lrzip version %d.%d file.\n", control->major_version, control->minor_version); } while (0); } while (0);
 ...
 if (control->major_version == 0 && control->minor_version < 4) {
  ...
  expected_size = __uint64_identity (expected_size);
 }
 ...
 if (md5 && (1)) {
  if (md5 == 1)
   ...
   do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 229, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Unknown hash, falling back to CRC\n"); } while (0); } while (0);
 }
 encrypted = magic[22];
 if (encrypted) {
  if (encrypted == 1)
   ...
   do { failure(control, 236, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Unknown encryption\n"); return 0; } while (0);
  ...
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 243, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Encryption hash loops %lld\n", control->encloops); } while (0); } while (0);
 } else if ((control->flags & (1 << 23))) {
  do { do { print_stuff(control, 1, 245, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Asked to decrypt a non-encrypted archive. Bypassing decryption.\n"); } while (0); } while (0);
  control->flags &= ~(1 << 23);
 }
 return 1;
}
...
_Bool read_magic(rzip_control *control, int fd_in, i64 *expected_size)
{
 ...
 if (__builtin_expect(!!(read(fd_in, magic, 24) != 24), 0))
  do { fatal(control, 258, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read magic header\n"); return 0; } while (0);

 if (__builtin_expect(!!(!get_magic(control, magic)), 0))
  return 0;
...
static _Bool preserve_perms(rzip_control *control, int fd_in, int fd_out)
{
 struct stat st;

 if (__builtin_expect(!!(fstat(fd_in, &st)), 0))
  do { fatal(control, 272, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to fstat input file\n"); return 0; } while (0);
 if (__builtin_expect(!!(fchmod(fd_out, (st.st_mode & 0666))), 0))
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 274, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Warning, unable to set permissions on %s\n", control->outfile); } while (0); } while (0);


 if (__builtin_expect(!!(fchown(fd_out, st.st_uid, st.st_gid)), 0))
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 278, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Warning, unable to set owner on %s\n", control->outfile); } while (0); } while (0);
 return 1;
}

static _Bool preserve_times(rzip_control *control, int fd_in)
{
 struct utimbuf times;
 struct stat st;

 if (__builtin_expect(!!(fstat(fd_in, &st)), 0))
  do { fatal(control, 288, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to fstat input file\n"); return 0; } while (0);
 times.actime = 0;
 times.modtime = st.st_mtim.tv_sec;
 if (__builtin_expect(!!(utime(control->outfile, &times)), 0))
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 292, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Warning, unable to set time on %s\n", control->outfile); } while (0); } while (0);
...
int open_tmpoutfile(rzip_control *control)
{
 ...
 if ((control->flags & (1 << 13)) && !(control->flags & (1 << 2)))
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 303, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Outputting to stdout.\n"); } while (0); } while (0);
 if (control->tmpdir) {
  control->outfile = realloc(((void*)0), strlen(control->tmpdir) + 16);
  if (__builtin_expect(!!(!control->outfile), 0))
   do { fatal(control, 307, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate outfile name\n"); return -1; } while (0);
 ...
 if (fd_out == -1) {
  do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 315, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "WARNING: Failed to create out tmpfile: %s, will fail if cannot perform %scompression entirely in ram\n", control->outfile, (control->flags & (1 << 4)) ? "de" : ""); } while (0); } while (0);

 } else
  register_outfile(control, control->outfile, (control->flags & (1 << 2)) || (control->flags & (1 << 13)) || !(control->flags & (1 << 19)));
 return fd_out;
}

static _Bool fwrite_stdout(rzip_control *control, void *buf, i64 len)
{
 ...
 while (len > 0) {
  ...
  if (__builtin_expect(!!(ret <= 0), 0))
   do { fatal(control, 335, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to fwrite in fwrite_stdout\n"); return 0; } while (0);
  len -= ret;
  offset_buf += ret;
  total += ret;
 }
...
_Bool write_fdout(rzip_control *control, void *buf, i64 len)
{
 ...
 while (len > 0) {
  ...
  if (__builtin_expect(!!(ret <= 0), 0))
   do { fatal(control, 353, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to write to fd_out in write_fdout\n"); return 0; } while (0);
  len -= ret;
  offset_buf += ret;
 }
 return 1;
...
{
 if (!(control->flags & (1 << 2))) {
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 363, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Dumping buffer to physical file.\n"); } while (0); } while (0);
  if ((control->flags & (1 << 13))) {
   if (__builtin_expect(!!(!fwrite_stdout(control, control->tmp_outbuf, control->out_len)), 0))
    return 0;
  } else {
...
_Bool dump_tmpoutfile(rzip_control *control, int fd_out)
{
 ...
 if (__builtin_expect(!!(fd_out == -1), 0))
  do { fatal(control, 384, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed: No temporary outfile created, unable to do in ram\n"); return 0; } while (0);

 fsync(fd_out);
 tmpoutfp = fdopen(fd_out, "r");
 if (__builtin_expect(!!(tmpoutfp == ((void*)0)), 0))
  do { fatal(control, 389, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to fdopen out tmpfile\n"); return 0; } while (0);
 rewind(tmpoutfp);

 if (!(control->flags & (1 << 2))) {
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 393, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Dumping temporary file to control->outFILE.\n"); } while (0); } while (0);
  while ((tmpchar = fgetc(tmpoutfp)) != (-1))
   putchar(tmpchar);
  fflush(control->outFILE);
  rewind(tmpoutfp);
 }

 if (__builtin_expect(!!(ftruncate(fd_out, 0)), 0))
  do { fatal(control, 401, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to ftruncate fd_out in dump_tmpoutfile\n"); return 0; } while (0);
 return 1;
}
...
{
 unsigned char *offset_buf = control->tmp_inbuf;
 ...
 while (len > 0) {
  ...
  if (__builtin_expect(!!(ret <= 0), 0))
   do { fatal(control, 417, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to write to fd_in in write_fdin\n"); return 0; } while (0);
  len -= ret;
  offset_buf += ret;
 }
 return 1;
...
int open_tmpinfile(rzip_control *control)
{
 ...
 if (control->tmpdir) {
  ...
  if (__builtin_expect(!!(!control->infile), 0))
   do { fatal(control, 433, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate infile name\n"); return -1; } while (0);
 ...
 if (fd_in == -1) {
  do { do { free((control->infile)); (control->infile) = ((void*)0); } while (0); control->infile = ((void*)0); } while (0);
  control->infile = malloc(16);
  if (__builtin_expect(!!(!control->infile), 0))
   do { fatal(control, 444, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate infile name\n"); return -1; } while (0);
 ...
 if (fd_in == -1) {
  ...
  if (__builtin_expect(!!(!control->infile), 0))
   do { fatal(control, 454, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate infile name\n"); return -1; } while (0);
  strcpy(control->infile, "/tmp/lrzipin.XXXXXX");
  fd_in = mkstemp(control->infile);
 }

 if (fd_in == -1) {
  do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 461, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "WARNING: Failed to create in tmpfile: %s, will fail if cannot perform %scompression entirely in ram\n", control->infile, (control->flags & (1 << 4)) ? "de" : ""); } while (0); } while (0);

 } else {
  register_infile(control, control->infile, ((control->flags & (1 << 4)) || (control->flags & (1 << 2))) && (control->flags & (1 << 12)));


  if (__builtin_expect(!!(unlink(control->infile)), 0)) {
   fatal(control, 467, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to unlink tmpfile: %s\n", control->infile);
   close(fd_in);
   return -1;
  }
 }
...
static _Bool read_tmpinmagic(rzip_control *control)
{
 char magic[(24)];
 int i, tmpchar;
 ...
 for (i = 0; i < 24; i++) {
  ...
  if (__builtin_expect(!!(tmpchar == (-1)), 0))
   do { failure(control, 484, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Reached end of file on STDIN prematurely on v05 magic read\n"); return 0; } while (0);
...
_Bool read_tmpinfile(rzip_control *control, int fd_in)
{
 ...
 if (__builtin_expect(!!(tmpinfp == ((void*)0)), 0))
  do { fatal(control, 502, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to fdopen in tmpfile\n"); return 0; } while (0);
...
static _Bool open_tmpoutbuf(rzip_control *control)
{
 ...
 while (42) {
  ...
  buf = malloc(maxlen);
  if (buf) {
   do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 523, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Malloced %""l" "d"" for tmp_outbuf\n", maxlen); } while (0); } while (0);
   break;
  }
  maxlen = maxlen / 3 * 2;
  if (maxlen < 100000000)
   do { fatal(control, 528, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Unable to even malloc 100MB for tmp_outbuf\n"); return 0; } while (0);
 }
 control->flags |= (1 << 21);
 ...
 if (!(control->flags & (1 << 4)) && !(control->flags & (1 << 2)))
  control->out_ofs = control->out_len = (24); return 1;
...
{
 control->flags &= ~(1 << 21);
 do { do { free((control->tmp_outbuf)); (control->tmp_outbuf) = ((void*)0); } while (0); control->tmp_outbuf = ((void*)0); } while (0);
 if (!(sizeof(long) == 4))
  control->usable_ram = control->maxram += control->ramsize / 18;
}

static _Bool open_tmpinbuf(rzip_control *control)
{
 control->flags |= (1 << 22);
 control->in_maxlen = control->maxram;
 control->tmp_inbuf = malloc(control->maxram + control->page_size);
 if (__builtin_expect(!!(!control->tmp_inbuf), 0))
  do { fatal(control, 557, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to malloc tmp_inbuf in open_tmpinbuf\n"); return 0; } while (0);
 return 1;
}
...
{
 if (__builtin_expect(!!(lseek(control->fd_in, 0, 0)), 0))
  do { fatal(control, 569, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to lseek on fd_in in clear_tmpinfile\n"); return 0; } while (0);
 if (__builtin_expect(!!(ftruncate(control->fd_in, 0)), 0))
  do { fatal(control, 571, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to truncate fd_in in clear_tmpinfile\n"); return 0; } while (0);
 return 1;
}
...
{
 control->flags &= ~(1 << 22);
 do { do { free((control->tmp_inbuf)); (control->tmp_inbuf) = ((void*)0); } while (0); control->tmp_inbuf = ((void*)0); } while (0);
 if (!(sizeof(long) == 4))
  control->usable_ram = control->maxram += control->ramsize / 18;
}
...
static int get_pass(rzip_control *control, char *s)
{
 ...
 if (control->passphrase)
  strncpy(s, control->passphrase, 512 - 8 - 1);
 else if (__builtin_expect(!!(fgets(s, 512 - 8, stdin) == ((void*)0)), 0))
  do { failure(control, 592, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to retrieve passphrase\n"); return -1; } while (0);
 len = strlen(s);
 if (len > 0 && ('\r' == s[len - 1] || '\n' == s[len - 1]))
  s[len - 1] = '\0';
 if (len > 1 && ('\r' == s[len - 2] || '\n' == s[len - 2]))
 ...
 if (__builtin_expect(!!(0 == len), 0))
  do { failure(control, 600, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Empty passphrase\n"); return -1; } while (0);
 return len;
}

static _Bool get_hash(rzip_control *control, int make_hash)
{
 char *passphrase, *testphrase;
 struct termios termios_p;
 int prompt = control->passphrase == ((void*)0);

 passphrase = calloc(512, 1);
 testphrase = calloc(512, 1);
 control->salt_pass = calloc(512, 1);
 control->hash = calloc(64, 1);
 if (__builtin_expect(!!(!passphrase || !testphrase || !control->salt_pass || !control->hash), 0)) {
  fatal(control, 615, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to calloc encrypt buffers in compress_file\n");
  do { do { free((testphrase)); (testphrase) = ((void*)0); } while (0); testphrase = ((void*)0); } while (0);
  do { do { free((passphrase)); (passphrase) = ((void*)0); } while (0); passphrase = ((void*)0); } while (0);
  return 0;
 }
 mlock(passphrase, 512);
 mlock(testphrase, 512);
 mlock(control->salt_pass, 512);
 mlock(control->hash, 64);

 if (control->pass_cb) {
  control->pass_cb(control->pass_data, passphrase, 512 - 8);
  if (!passphrase[0]) {
   fatal(control, 628, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Supplied password was null!");
   munlock(passphrase, 512);
   munlock(testphrase, 512);
   do { do { free((testphrase)); (testphrase) = ((void*)0); } while (0); testphrase = ((void*)0); } while (0);
   do { do { free((passphrase)); (passphrase) = ((void*)0); } while (0); passphrase = ((void*)0); } while (0);
   release_hashes(control);
   return 0;
  }
  control->salt_pass_len = strlen(passphrase) + 8;
 } else {

  tcgetattr(fileno(stdin), &termios_p);
  termios_p.c_lflag &= ~0000010;
  tcsetattr(fileno(stdin), 0, &termios_p);
retry_pass:
  if (prompt)
   do { do { print_stuff(control, 1, 644, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Enter passphrase: "); } while (0); } while (0);
  control->salt_pass_len = get_pass(control, passphrase) + 8;
  if (prompt)
   do { do { print_stuff(control, 1, 647, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "\n"); } while (0); } while (0);
  if (make_hash) {
   if (prompt)
    do { do { print_stuff(control, 1, 650, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Re-enter passphrase: "); } while (0); } while (0);
   get_pass(control, testphrase);
   if (prompt)
    do { do { print_stuff(control, 1, 653, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "\n"); } while (0); } while (0);
   if (strcmp(passphrase, testphrase)) {
    do { do { print_stuff(control, 1, 655, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Passwords do not match. Try again.\n"); } while (0); } while (0);
    goto retry_pass;
   }
  }
  termios_p.c_lflag |= 0000010;
  tcsetattr(fileno(stdin), 0, &termios_p);
  memset(testphrase, 0, 512);
 }
 memcpy(control->salt_pass, control->salt, 8);
 memcpy(control->salt_pass + 8, passphrase, 512 - 8);
 lrz_stretch(control);
...
static void release_hashes(rzip_control *control)
{
 memset(control->salt_pass, 0, 512);
 memset(control->hash, 0, 8);
 munlock(control->salt_pass, 512);
 munlock(control->hash, 64);
 do { do { free((control->salt_pass)); (control->salt_pass) = ((void*)0); } while (0); control->salt_pass = ((void*)0); } while (0);
 do { do { free((control->hash)); (control->hash) = ((void*)0); } while (0); control->hash = ((void*)0); } while (0);
}
...
{
 char *tmp, *tmpoutfile, *infilecopy = ((void*)0);
 int fd_in, fd_out = -1, fd_hist = -1;
 i64 expected_size = 0, free_space;
 struct statvfs fbuf;

 if (!(control->flags & (1 << 12)) && !( !!(control->inFILE) && !(control->flags & (1 << 12)) )) {
  struct stat fdin_stat;

  stat(control->infile, &fdin_stat);
  if (!((((fdin_stat.st_mode)) & 0170000) == (0100000)) && (tmp = strrchr(control->infile, '.')) &&
      strcmp(tmp,control->suffix)) {



   infilecopy = __builtin_alloca (strlen(control->infile) + strlen(control->suffix) + 1);
   strcpy(infilecopy, control->infile);
   strcat(infilecopy, control->suffix);
  } else
   infilecopy = (__extension__ ({ const char *__old = (control->infile); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); }));
 ...
 if (!(control->flags & (1 << 13)) && !(control->flags & (1 << 2))) {
  ...
  if (control->outname) {
   ...
   if (control->outdir && (tmp = strrchr(infilecopy, '/')))
    tmpoutfile = (__extension__ ({ const char *__old = (tmp + 1); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); }));
   else
    tmpoutfile = (__extension__ ({ const char *__old = (infilecopy); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); }));


   if ((tmp = strrchr(tmpoutfile, '.')) && !strcmp(tmp, control->suffix))
    *tmp='\0';

   control->outfile = malloc((control->outdir == ((void*)0)? 0: strlen(control->outdir)) + strlen(tmpoutfile) + 1);
   if (__builtin_expect(!!(!control->outfile), 0))
    do { fatal(control, 731, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate outfile name\n"); return 0; } while (0);
  ...
  if (!(control->flags & (1 << 13)))
   do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 741, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Output filename is: %s\n", control->outfile); } while (0); } while (0);
 }

 if ( ( !!(control->inFILE) && !(control->flags & (1 << 12)) ) ) {
  fd_in = fileno(control->inFILE);
 }
 else if ((control->flags & (1 << 12))) {
  fd_in = open_tmpinfile(control);
  read_tmpinmagic(control);
  if ((control->flags & (1 << 23)))
   do { failure(control, 751, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Cannot decompress encrypted file from STDIN\n"); return 0; } while (0);
  expected_size = control->st_size;
  if (__builtin_expect(!!(!open_tmpinbuf(control)), 0))
   return 0;
 } else {
  fd_in = open(infilecopy, 00);
  if (__builtin_expect(!!(fd_in == -1), 0)) {
   do { fatal(control, 758, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to open %s\n", infilecopy); return 0; } while (0);
  }
 }
 control->fd_in = fd_in;

 if (!((control->flags & (1 << 2)) | (control->flags & (1 << 13)))) {
  fd_out = open(control->outfile, 01 | 0100 | 0200, 0666);
  if ((control->flags & (1 << 3)) && (-1 == fd_out) && (17 == (*__errno_location ()))) {
   if (__builtin_expect(!!(unlink(control->outfile)), 0))
    do { fatal(control, 767, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to unlink an existing file: %s\n", control->outfile); return 0; } while (0);
   fd_out = open(control->outfile, 01 | 0100 | 0200, 0666);
  }
  if (__builtin_expect(!!(fd_out == -1), 0)) {
   ...
   control->flags |= (1 << 19);
   do { fatal(control, 774, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to create %s\n", control->outfile); return 0; } while (0);
  }
  fd_hist = open(control->outfile, 00);
  if (__builtin_expect(!!(fd_hist == -1), 0))
   do { fatal(control, 778, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to open history file %s\n", control->outfile); return 0; } while (0);


  if (!(control->flags & (1 << 12)))
   if (__builtin_expect(!!(!preserve_perms(control, fd_in, fd_out)), 0))
    return 0;
 } else {
  ...
  if (fd_out == -1) {
   ...
   fd_hist = open(control->outfile, 00);
   if (__builtin_expect(!!(fd_hist == -1), 0))
    do { fatal(control, 791, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to open history file %s\n", control->outfile); return 0; } while (0);

   if (__builtin_expect(!!(unlink(control->outfile)), 0))
    do { fatal(control, 794, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to unlink tmpfile: %s\n", control->outfile); return 0; } while (0);
  }
 }

 if ((control->flags & (1 << 13))) {
  if (__builtin_expect(!!(!open_tmpoutbuf(control)), 0))
   return 0;
 }

 if (!(control->flags & (1 << 12))) {
  if (__builtin_expect(!!(!read_magic(control, fd_in, &expected_size)), 0))
   return 0;
 }
 ...
 if (!(control->flags & (1 << 13)) && !(control->flags & (1 << 2))) {
  ...
  if (__builtin_expect(!!(fstatvfs(fd_out, &fbuf)), 0))
   do { fatal(control, 812, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to fstatvfs in decompress_file\n"); return 0; } while (0);
  free_space = (i64)fbuf.f_bsize * (i64)fbuf.f_bavail;
  if (free_space < expected_size) {
   if ((control->flags & (1 << 3)))
    do { print_err(control, 816, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Warning, inadequate free space detected, but attempting to decompress due to -f option being used.\n"); } while (0);
   else
    do { failure(control, 818, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Inadequate free space to decompress file, use -f to override.\n"); return 0; } while (0);
  }
 }
 ...
 if ((!((control->flags & (1 << 16))) && !((control->flags & (1 << 17)))))
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 825, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Not performing MD5 hash check\n"); } while (0); } while (0);
 if ((control->flags & (1 << 17)))
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 827, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "MD5 "); } while (0); } while (0);
 else
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 829, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "CRC32 "); } while (0); } while (0);
 do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 830, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "being used for integrity testing.\n"); } while (0); } while (0);

 if ((control->flags & (1 << 23)))
  if (__builtin_expect(!!(!get_hash(control, 0)), 0))
   return 0;

 do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 836, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Decompressing...\n"); } while (0); } while (0);

 if (__builtin_expect(!!(runzip_fd(control, fd_in, fd_out, fd_hist, expected_size) < 0), 0))
  return 0;

 if ((control->flags & (1 << 13)) && !(control->flags & (1 << 21))) {
  if (__builtin_expect(!!(!dump_tmpoutfile(control, fd_out)), 0))
   return 0;
 }


 do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 847, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "\r"); } while (0); } while (0);
 if (!((control->flags & (1 << 13)) | (control->flags & (1 << 2))))
  do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 849, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Output filename is: %s: ", control->outfile); } while (0); } while (0);
 if (!expected_size)
  expected_size = control->st_size;
 if (!(control->flags & (1 << 23)))
  do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 853, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "[OK] - %lld bytes                                \n", expected_size); } while (0); } while (0);
 else
  do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 855, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "[OK]                                             \n"); } while (0); } while (0);

 if ((control->flags & (1 << 21)))
  close_tmpoutbuf(control);

 if (fd_out > 0) {
  if (__builtin_expect(!!(close(fd_hist) || close(fd_out)), 0))
   do { fatal(control, 862, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to close files\n"); return 0; } while (0);
 }

 if (__builtin_expect(!!(!(control->flags & (1 << 12)) && !(control->flags & (1 << 13)) && !(control->flags & (1 << 2)) && !preserve_times(control, fd_in)), 0))
  return 0;
 ...
 if (!(control->flags & (1 << 1)) && !(control->flags & (1 << 12))) {
  if (__builtin_expect(!!(unlink(control->infile)), 0))
   do { fatal(control, 874, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to unlink %s\n", infilecopy); return 0; } while (0);
 }

 if ((control->flags & (1 << 23)))
  release_hashes(control);

 do { do { free((control->outfile)); (control->outfile) = ((void*)0); } while (0); control->outfile = ((void*)0); } while (0);
 return 1;
}

_Bool get_header_info(rzip_control *control, int fd_in, unsigned char *ctype, i64 *c_len,
       i64 *u_len, i64 *last_head, int chunk_bytes)
{
 if (__builtin_expect(!!(read(fd_in, ctype, 1) != 1), 0))
  do { fatal(control, 888, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read in get_header_info\n"); return 0; } while (0);

 *c_len = *u_len = *last_head = 0;
 if (control->major_version == 0 && control->minor_version < 4) {
  u32 c_len32, u_len32, last_head32;

  if (__builtin_expect(!!(read(fd_in, &c_len32, 4) != 4), 0))
   do { fatal(control, 895, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read in get_header_info"); return 0; } while (0);
  if (__builtin_expect(!!(read(fd_in, &u_len32, 4) != 4), 0))
   do { fatal(control, 897, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read in get_header_info"); return 0; } while (0);
  if (__builtin_expect(!!(read(fd_in, &last_head32, 4) != 4), 0))
   do { fatal(control, 899, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read in get_header_info"); return 0; } while (0);
  c_len32 = __uint32_identity (c_len32);
  u_len32 = __uint32_identity (u_len32);
  last_head32 = __uint32_identity (last_head32);
  *c_len = c_len32;
  *u_len = u_len32;
  *last_head = last_head32;
  ...
  if (__builtin_expect(!!(read(fd_in, c_len, read_len) != read_len), 0))
   do { fatal(control, 914, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read in get_header_info"); return 0; } while (0);
  if (__builtin_expect(!!(read(fd_in, u_len, read_len) != read_len), 0))
   do { fatal(control, 916, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read in get_header_info"); return 0; } while (0);
  if (__builtin_expect(!!(read(fd_in, last_head, read_len) != read_len), 0))
   do { fatal(control, 918, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read_i64 in get_header_info"); return 0; } while (0);
...
_Bool get_fileinfo(rzip_control *control)
{
 ...
 if ( ( !!(control->inFILE) && !(control->flags & (1 << 12)) ) )
  ...
  fd_in = open(infilecopy, 00);
  if (__builtin_expect(!!(fd_in == -1), 0))
   do { fatal(control, 974, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to open %s\n", infilecopy); return 0; } while (0);
 }


 if (__builtin_expect(!!(fstat(fd_in, &st)), 0))
  do { fatal(control, 979, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "bad magic file descriptor!?\n"); goto error; } while (0);
 ...
 if ((control->flags & (1 << 23))) {
  do { do { print_stuff(control, 1, 987, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Encrypted lrzip archive. No further information available\n"); } while (0); } while (0);
  if (!(control->flags & (1 << 12)) && !( !!(control->inFILE) && !(control->flags & (1 << 12)) ))
   close(fd_in);
  goto out;
 }
 ...
 if (control->major_version == 0 && control->minor_version > 4) {
  if (__builtin_expect(!!(read(fd_in, &chunk_byte, 1) != 1), 0))
   do { fatal(control, 995, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read chunk_byte in get_fileinfo\n"); goto error; } while (0);
  if (__builtin_expect(!!(chunk_byte < 1 || chunk_byte > 8), 0))
   do { fatal(control, 997, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Invalid chunk bytes %d\n", chunk_byte); goto error; } while (0);
  if (control->major_version == 0 && control->minor_version > 5) {
   if (__builtin_expect(!!(read(fd_in, &control->eof, 1) != 1), 0))
    do { fatal(control, 1000, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read eof in get_fileinfo\n"); goto error; } while (0);
   if (__builtin_expect(!!(read(fd_in, &chunk_size, chunk_byte) != chunk_byte), 0))
    do { fatal(control, 1002, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read chunk_size in get_fileinfo\n"); goto error; } while (0);
   chunk_size = __uint64_identity (chunk_size);
   if (__builtin_expect(!!(chunk_size < 0), 0))
    do { fatal(control, 1005, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Invalid chunk size %lld\n", chunk_size); goto error; } while (0);
  }
 }
...
next_chunk:
 ...
 do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1030, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Rzip chunk %d:\n", ++chunk); } while (0); } while (0);
 if (chunk_byte)
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1032, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Chunk byte width: %d\n", chunk_byte); } while (0); } while (0);
 if (chunk_size) {
  chunk_total += chunk_size;
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1035, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Chunk size: %lld\n", chunk_size); } while (0); } while (0);
 }
 if (__builtin_expect(!!(chunk_byte && (chunk_byte > 8 || chunk_size < 0)), 0))
  failure(control, 1038, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Invalid chunk data\n");
 while (stream < 2) {
  int block = 1;

  second_last = 0;
  if (__builtin_expect(!!(lseek(fd_in, stream_head[stream] + ofs, 0) == -1), 0))
   do { fatal(control, 1044, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to seek to header data in get_fileinfo\n"); goto error; } while (0);
  if (__builtin_expect(!!(!get_header_info(control, fd_in, &ctype, &c_len, &u_len, &last_head, chunk_byte)), 0))
   return 0;

  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1048, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Stream: %d\n", stream); } while (0); } while (0);
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 1049, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Offset: %lld\n", ofs); } while (0); } while (0);
  do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1050, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Block\tComp\tPercent\tSize\n"); } while (0); } while (0);
  do {
   i64 head_off;

   if (__builtin_expect(!!(last_head && last_head < second_last), 0))
    do { failure(control, 1055, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Invalid earlier last_head position, corrupt archive.\n"); goto error; } while (0);
   second_last = last_head;
   if (__builtin_expect(!!(last_head + ofs > infile_size), 0))
    do { failure(control, 1058, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Offset greater than archive size, likely corrupted/truncated archive.\n"); goto error; } while (0);
   if (__builtin_expect(!!(head_off = lseek(fd_in, last_head + ofs, 0) == -1), 0))
    do { fatal(control, 1060, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to seek to header data in get_fileinfo\n"); goto error; } while (0);
   if (__builtin_expect(!!(!get_header_info(control, fd_in, &ctype, &c_len, &u_len, &last_head, chunk_byte)), 0))

    return 0;
   if (__builtin_expect(!!(last_head < 0 || c_len < 0 || u_len < 0), 0))
    do { failure(control, 1065, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Entry negative, likely corrupted archive.\n"); goto error; } while (0);
   do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1066, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "%d\t", block); } while (0); } while (0);
   if (ctype == 3)
    do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1068, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "none"); } while (0); } while (0);
   else if (ctype == 4)
    do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1070, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "bzip2"); } while (0); } while (0);
   else if (ctype == 5)
    do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1072, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "lzo"); } while (0); } while (0);
   else if (ctype == 6)
    do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1074, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "lzma"); } while (0); } while (0);
   else if (ctype == 7)
    do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1076, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "gzip"); } while (0); } while (0);
   else if (ctype == 8)
    do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1078, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "zpaq"); } while (0); } while (0);
   else
    do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1080, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Dunno wtf"); } while (0); } while (0);
   utotal += u_len;
   ctotal += c_len;
   do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1083, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "\t%.1f%%\t%lld / %lld", percentage(c_len, u_len), c_len, u_len); } while (0); } while (0);
   do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 1084, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "\tOffset: %lld\tHead: %lld", head_off, last_head); } while (0); } while (0);
   do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1085, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "\n"); } while (0); } while (0);
   block++;
  } while (last_head);
  ++stream;
 }

 if (__builtin_expect(!!((ofs = lseek(fd_in, c_len, 1)) == -1), 0))
  do { fatal(control, 1092, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to lseek c_len in get_fileinfo\n"); goto error; } while (0);

 if (ofs >= infile_size - ((control->flags & (1 << 17)) ? 16 : 0))
  goto done;

 if (control->major_version == 0 && control->minor_version > 4) {
  if (__builtin_expect(!!(read(fd_in, &chunk_byte, 1) != 1), 0))
   do { fatal(control, 1099, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read chunk_byte in get_fileinfo\n"); goto error; } while (0);
  if (__builtin_expect(!!(chunk_byte < 1 || chunk_byte > 8), 0))
   do { fatal(control, 1101, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Invalid chunk bytes %d\n", chunk_byte); goto error; } while (0);
  ofs++;
  if (control->major_version == 0 && control->minor_version > 5) {
   if (__builtin_expect(!!(read(fd_in, &control->eof, 1) != 1), 0))
    do { fatal(control, 1105, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read eof in get_fileinfo\n"); goto error; } while (0);
   if (__builtin_expect(!!(read(fd_in, &chunk_size, chunk_byte) != chunk_byte), 0))
    do { fatal(control, 1107, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read chunk_size in get_fileinfo\n"); goto error; } while (0);
   chunk_size = __uint64_identity (chunk_size);
   if (__builtin_expect(!!(chunk_size < 0), 0))
    do { fatal(control, 1110, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Invalid chunk size %lld\n", chunk_size); goto error; } while (0);
   ofs += 1 + chunk_byte;
   header_length = 1 + (chunk_byte * 3);
  }
 }
 ...
 if (__builtin_expect(!!(ofs > infile_size), 0))
  do { failure(control, 1118, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Offset greater than archive size, likely corrupted/truncated archive.\n"); goto error; } while (0);
 do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1121, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Rzip compression: %.1f%% %lld / %lld\n", percentage (utotal, expected_size), utotal, expected_size); } while (0); } while (0);


 do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1124, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Back end compression: %.1f%% %lld / %lld\n", percentage(ctotal, utotal), ctotal, utotal); } while (0); } while (0);


 do { if ((control->flags & ((1 << 10) | (1 << 11)))) do { print_stuff(control, 3, 1127, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Overall compression: %.1f%% %lld / %lld\n", percentage(ctotal, expected_size), ctotal, expected_size); } while (0); } while (0);



 cratio = (long double)expected_size / (long double)infile_size;

 do { do { print_stuff(control, 1, 1131, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "%s:\nlrzip version: %d.%d file\n", infilecopy, control->major_version, control->minor_version); } while (0); } while (0);

 do { do { print_stuff(control, 1, 1133, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Compression: "); } while (0); } while (0);
 if (ctype == 3)
  do { do { print_stuff(control, 1, 1135, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "rzip alone\n"); } while (0); } while (0);
 else if (ctype == 4)
  do { do { print_stuff(control, 1, 1137, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "rzip + bzip2\n"); } while (0); } while (0);
 else if (ctype == 5)
  do { do { print_stuff(control, 1, 1139, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "rzip + lzo\n"); } while (0); } while (0);
 else if (ctype == 6)
  do { do { print_stuff(control, 1, 1141, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "rzip + lzma\n"); } while (0); } while (0);
 else if (ctype == 7)
  do { do { print_stuff(control, 1, 1143, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "rzip + gzip\n"); } while (0); } while (0);
 else if (ctype == 8)
  do { do { print_stuff(control, 1, 1145, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "rzip + zpaq\n"); } while (0); } while (0);
 else
  do { do { print_stuff(control, 1, 1147, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Dunno wtf\n"); } while (0); } while (0);
 do { do { print_stuff(control, 1, 1148, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Decompressed file size: %llu\n", expected_size); } while (0); } while (0);
 do { do { print_stuff(control, 1, 1149, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Compressed file size: %llu\n", infile_size); } while (0); } while (0);
 do { do { print_stuff(control, 1, 1150, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Compression ratio: %.3Lf\n", cratio); } while (0); } while (0);

 if ((control->flags & (1 << 17))) {
  char md5_stored[16];
  int i;

  do { do { print_stuff(control, 1, 1156, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "MD5 used for integrity testing\n"); } while (0); } while (0);
  if (__builtin_expect(!!(lseek(fd_in, -16, 2) == -1), 0))
   do { fatal(control, 1158, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to seek to md5 data in runzip_fd\n"); goto error; } while (0);
  if (__builtin_expect(!!(read(fd_in, md5_stored, 16) != 16), 0))
   do { fatal(control, 1160, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to read md5 data in runzip_fd\n"); goto error; } while (0);
  do { do { print_stuff(control, 1, 1161, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "MD5: "); } while (0); } while (0);
  for (i = 0; i < 16; i++)
   do { do { print_stuff(control, 1, 1163, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "%02x", md5_stored[i] & 0xFF); } while (0); } while (0);
  do { do { print_stuff(control, 1, 1164, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "\n"); } while (0); } while (0);
 } else
  do { do { print_stuff(control, 1, 1166, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "CRC32 used for integrity testing\n"); } while (0); } while (0);
 if ( !( !!(control->inFILE) && !(control->flags & (1 << 12)) ) )
  if (__builtin_expect(!!(close(fd_in)), 0))
   do { fatal(control, 1169, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to close fd_in in get_fileinfo\n"); return 0; } while (0);

out:
 do { do { free((control->outfile)); (control->outfile) = ((void*)0); } while (0); control->outfile = ((void*)0); } while (0);
 return 1;
error:
 if (!(control->flags & (1 << 12)) && ! ( !!(control->inFILE) && !(control->flags & (1 << 12)) )) close(fd_in);
 return 0;
}
...
_Bool compress_file(rzip_control *control)
{
 ...
 if ((1))
  control->flags |= (1 << 17);
 if ((control->flags & (1 << 23)))
  if (__builtin_expect(!!(!get_hash(control, 1)), 0))
   return 0;
 ...
 if ( ( !!(control->inFILE) && !(control->flags & (1 << 12)) ) )
  ...
  if ((tmp = strrchr(control->infile, '.')) && !strcmp(tmp, control->suffix)) {
   do { print_err(control, 1202, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "%s: already has %s suffix. Skipping...\n", control->infile, control->suffix); } while (0);
   return 0;
  }

        fd_in = open(control->infile, 00);
  if (__builtin_expect(!!(fd_in == -1), 0))
   do { fatal(control, 1208, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to open %s\n", control->infile); return 0; } while (0);
 }
 else
  fd_in = 0;
 ...
 if (!(control->flags & (1 << 13))) {
  if (control->outname) {
    ...
    if (*(control->suffix) == '\0')
     ...
     if (__builtin_expect(!!(!control->outfile), 0))
      do { fatal(control, 1221, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate outfile name\n"); goto error; } while (0);
     strcpy(control->outfile, control->outname);
     strcat(control->outfile, control->suffix);
     do { do { print_stuff(control, 1, 1224, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Suffix added to %s.\nFull pathname is: %s\n", control->outname, control->outfile); } while (0); } while (0);
    } else
     control->outfile = strdup(control->outname);
  } else {
   ...
   control->outfile = malloc((control->outdir == ((void*)0)? 0: strlen(control->outdir)) + strlen(tmpinfile) + strlen(control->suffix) + 1);
   if (__builtin_expect(!!(!control->outfile), 0))
    do { fatal(control, 1239, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate outfile name\n"); goto error; } while (0);
   ...
   strcat(control->outfile, control->suffix);
   do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 1247, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Output filename is: %s\n", control->outfile); } while (0); } while (0);
  }

  fd_out = open(control->outfile, 02 | 0100 | 0200, 0666);
  if ((control->flags & (1 << 3)) && (-1 == fd_out) && (17 == (*__errno_location ()))) {
   if (__builtin_expect(!!(unlink(control->outfile)), 0))
    do { fatal(control, 1253, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to unlink an existing file: %s\n", control->outfile); goto error; } while (0);
   fd_out = open(control->outfile, 02 | 0100 | 0200, 0666);
  }
  if (__builtin_expect(!!(fd_out == -1), 0)) {
   ...
   control->flags |= (1 << 19);
   do { fatal(control, 1260, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to create %s\n", control->outfile); goto error; } while (0);
  }
  control->fd_out = fd_out;
  if (!(control->flags & (1 << 12))) {
   if (__builtin_expect(!!(!preserve_perms(control, fd_in, fd_out)), 0))
 ...
 if (__builtin_expect(!!(!(control->flags & (1 << 13)) && write(fd_out, header, sizeof(header)) != sizeof(header)), 0))
  do { fatal(control, 1274, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Cannot write file header\n"); goto error; } while (0);

 rzip_fd(control, fd_in, fd_out);


 if (!(control->flags & (1 << 13))) {
  if (__builtin_expect(!!(!write_magic(control)), 0))
   goto error;
 }
 ...
 if (__builtin_expect(!!(!(control->flags & (1 << 12)) && !(control->flags & (1 << 13)) && !preserve_times(control, fd_in)), 0)) {
  fatal(control, 1288, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to preserve times on output file\n");
  goto error;
 }

 if (__builtin_expect(!!(close(fd_in)), 0)) {
  fatal(control, 1293, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to close fd_in\n");
  fd_in = -1;
  goto error;
 }
 if (__builtin_expect(!!(!(control->flags & (1 << 13)) && close(fd_out)), 0))
  do { fatal(control, 1298, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to close fd_out\n"); return 0; } while (0);
 if ((control->flags & (1 << 21)))
  close_tmpoutbuf(control);

 if (!(control->flags & (1 << 1)) && !(control->flags & (1 << 12))) {
  if (__builtin_expect(!!(unlink(control->infile)), 0))
   do { fatal(control, 1304, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to unlink %s\n", control->infile); return 0; } while (0);
 }

 do { do { free((control->outfile)); (control->outfile) = ((void*)0); } while (0); control->outfile = ((void*)0); } while (0);
 return 1;
error:
 if (! ( !!(control->inFILE) && !(control->flags & (1 << 12)) ) && (control->flags & (1 << 12)) && (fd_in > 0))
  close(fd_in);
 if ((!(control->flags & (1 << 13))) && (fd_out > 0))
  close(fd_out);
 return 0;
}
...
_Bool initialise_control(rzip_control *control)
{
 ...
 control->flags = (1 << 0) | (1 << 1) | (1 << 20);
 control->suffix = ".lrz";
 control->compression_level = 7;
 control->ramsize = get_ram(control);
 if (__builtin_expect(!!(control->ramsize == -1), 0))
  return 0;

 control->threads = (sysconf(_SC_NPROCESSORS_ONLN));
 control->page_size = (sysconf(_SC_PAGESIZE));
 control->nice_val = 19;




 if (__builtin_expect(!!((now_t = time(((void*)0))) == ((time_t)-1)), 0))
  do { fatal(control, 1342, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to call time in main\n"); return 0; } while (0);
 if (__builtin_expect(!!(now_t < 1293840000), 0)) {
  do { do { print_stuff(control, 1, 1344, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Warning your time reads before the year 2011, check your system clock\n"); } while (0); } while (0);
  now_t = 1293840000;
 }


 tdiff = (now_t - 1293840000) / 4;
 now_t = 1293840000 + tdiff;
 control->secs = now_t;
 control->encloops = nloops(control->secs, control->salt, control->salt + 1);
 if (__builtin_expect(!!(!get_rand(control, control->salt + 2, 6)), 0))
  return 0;
 ...
 if (control->tmpdir == ((void*)0))
  do { fatal(control, 1370, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/lrzip.c", __func__, "Failed to allocate for tmpdir\n"); return 0; } while (0);
