# 1 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 343 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
# 21 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c"
# 1 "./config.h" 1
# 22 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
...
# 25 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
...
# 27 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
...
# 30 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
...
# 33 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
...
# 41 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
...
# 44 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
# 1 "./runzip.h" 1
# 25 "./runzip.h"
i64 runzip_fd(rzip_control *control, int fd_in, int fd_out, int fd_hist, i64 expected_size);
# 45 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
# 1 "./stream.h" 1
# 26 "./stream.h"
...
# 46 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
# 1 "./util.h" 1
# 23 "./util.h"
...
# 47 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
# 1 "./lrzip_core.h" 1
# 24 "./lrzip_core.h"
...
extern void zpaq_decompress(unsigned char *s_buf, i64 *d_len, unsigned char *c_buf, i64 c_len,
       FILE *msgout, _Bool progress, long thread);
# 48 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2
...
# 50 "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c" 2

static inline unsigned char read_u8(rzip_control *control, void *ss, int stream, _Bool *err)
{
 unsigned char b;

 if (__builtin_expect(!!(read_stream(control, ss, stream, &b, 1) != 1), 0)) {
  *err = 1;
  do { fatal(control, 57, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Stream read u8 failed\n"); return 0; } while (0);
 }
 return b;
}

static inline u32 read_u32(rzip_control *control, void *ss, int stream, _Bool *err)
{
 u32 ret;

 if (__builtin_expect(!!(read_stream(control, ss, stream, (unsigned char *)&ret, 4) != 4), 0)) {
  *err = 1;
  do { fatal(control, 68, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Stream read u32 failed\n"); return 0; } while (0);
 }
 ret = __uint32_identity (ret);
 return ret;
}
...
static inline i64 read_vchars(rzip_control *control, void *ss, int stream, int length)
{
 ...
 if (__builtin_expect(!!(read_stream(control, ss, stream, (unsigned char *)&s, length) != length), 0))
  do { fatal(control, 80, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Stream read of %d bytes failed\n", length); return -1; } while (0);
...
static i64 seekcur_fdout(rzip_control *control)
{
 if (!(control->flags & (1 << 21)))
  return lseek(control->fd_out, 0, 1);
...
{
 if (!(control->flags & (1 << 21)))
  return lseek(control->fd_hist, pos, 0);
 control->hist_ofs = pos - control->out_relofs;
 if (control->hist_ofs > control->out_len)
  control->out_len = control->hist_ofs;
 if (__builtin_expect(!!(control->hist_ofs < 0 || control->hist_ofs > control->out_maxlen), 0)) {
  do { print_err(control, 100, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Trying to seek outside tmpoutbuf to %lld in seekto_fdhist\n", control->hist_ofs); } while (0);
  return -1;
 }
 return pos;
}
...
static i64 seekcur_fdin(rzip_control *control)
{
 if (!(control->flags & (1 << 22)))
  return lseek(control->fd_in, 0, 1);
...
{
 if (!(control->flags & (1 << 22)))
  return lseek(control->fd_in, pos, 0);
 if (__builtin_expect(!!(pos > control->in_len || pos < 0), 0)) {
  do { print_err(control, 118, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Trying to seek outside tmpinbuf to %lld in seekto_fdin\n", pos); } while (0);
  return -1;
 }
 control->in_ofs = pos;
 return 0;
...
static i64 seekto_fdinend(rzip_control *control)
{
 ...
 if (!(control->flags & (1 << 22)))
  return lseek(control->fd_in, 0, 2);
 while ((tmpchar = getchar()) != (-1)) {
  control->tmp_inbuf[control->in_len++] = (char)tmpchar;
  if (__builtin_expect(!!(control->in_len > control->in_maxlen), 0))
   do { failure(control, 134, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Trying to read greater than max_len\n"); return -1; } while (0);
 }
 control->in_ofs = control->in_len;
 return control->in_ofs;
}

static i64 read_header(rzip_control *control, void *ss, unsigned char *head)
{
 _Bool err = 0;
...
static i64 unzip_literal(rzip_control *control, void *ss, i64 len, unsigned int *cksum)
{
 i64 stream_read;
 unsigned char *buf;

 if (__builtin_expect(!!(len < 0), 0))
  do { failure(control, 156, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "len %lld is negative in unzip_literal!\n",len); return -1; } while (0);

 buf = (unsigned char *)malloc(len);
 if (__builtin_expect(!!(!buf), 0))
  do { fatal(control, 160, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to malloc literal buffer of size %lld\n", len); return -1; } while (0);

 stream_read = read_stream(control, ss, 1, buf, len);
 if (__builtin_expect(!!(stream_read == -1), 0)) {
  do { do { free((buf)); (buf) = ((void*)0); } while (0); buf = ((void*)0); } while (0);
  do { fatal(control, 165, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to read_stream in unzip_literal\n"); return -1; } while (0);
 }

 if (__builtin_expect(!!(write_1g(control, buf, (size_t)stream_read) != (ssize_t)stream_read), 0)) {
  do { do { free((buf)); (buf) = ((void*)0); } while (0); buf = ((void*)0); } while (0);
  do { fatal(control, 170, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to write literal buffer of size %lld\n", stream_read); return -1; } while (0);
 }
...
static i64 read_fdhist(rzip_control *control, void *buf, i64 len)
{
 ...
 if (__builtin_expect(!!(len + control->hist_ofs > control->out_maxlen), 0)) {
  do { print_err(control, 187, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Trying to read beyond end of tmpoutbuf in read_fdhist\n"); } while (0);
  return -1;
 }
...
static i64 unzip_match(rzip_control *control, void *ss, i64 len, unsigned int *cksum, int chunk_bytes)
{
 i64 offset, n, total, cur_pos;
 unsigned char *buf, *off_buf;

 if (__builtin_expect(!!(len < 0), 0))
  do { failure(control, 200, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "len %lld is negative in unzip_match!\n",len); return -1; } while (0);

 total = 0;
 cur_pos = seekcur_fdout(control);
 if (__builtin_expect(!!(cur_pos == -1), 0))
  do { fatal(control, 205, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Seek failed on out file in unzip_match.\n"); return -1; } while (0);


 offset = read_vchars(control, ss, 0, chunk_bytes);
 if (__builtin_expect(!!(offset == -1), 0))
  return -1;
 if (__builtin_expect(!!(seekto_fdhist(control, cur_pos - offset) == -1), 0))
  do { fatal(control, 213, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Seek failed by %d from %d on history file in unzip_match\n", offset, cur_pos); return -1; } while (0);


 buf = (unsigned char *)malloc(len);
 if (__builtin_expect(!!(!buf), 0))
  do { fatal(control, 217, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to malloc match buffer of size %lld\n", len); return -1; } while (0);
 off_buf = buf;

 while (len) {
  n = ((len) < (offset)? (len): (offset));
  if (__builtin_expect(!!(n < 1), 0))
   do { fatal(control, 223, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed fd history in unzip_match due to corrupt archive\n"); return -1; } while (0);

  if (__builtin_expect(!!(read_fdhist(control, off_buf, (size_t)n) != (ssize_t)n), 0)) {
   do { do { free((buf)); (buf) = ((void*)0); } while (0); buf = ((void*)0); } while (0);
   do { fatal(control, 227, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to read %d bytes in unzip_match\n", n); return -1; } while (0);
  }
  if (__builtin_expect(!!(write_1g(control, off_buf, (size_t)n) != (ssize_t)n), 0)) {
   do { do { free((buf)); (buf) = ((void*)0); } while (0); buf = ((void*)0); } while (0);
   do { fatal(control, 231, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to write %d bytes in unzip_match\n", n); return -1; } while (0);
  }
...
{
 unsigned int good_cksum, cksum = 0;
 i64 len, ofs, total = 0;
 int l = -1, p = 0;
 char chunk_bytes;
 struct stat st;
 ...
 if (expected_size > (i64)10737418240ULL)
  divisor_index = 3;
 else if (expected_size > 10485760)
  divisor_index = 2;
 else if (expected_size > 10240)
  divisor_index = 1;
 else
  divisor_index = 0;
 ...
 if (control->major_version == 0 && control->minor_version < 4)
  ...
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 288, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Reading chunk_bytes at %lld\n", get_readseek(control, fd_in)); } while (0); } while (0);

  if (__builtin_expect(!!(read_1g(control, fd_in, &chunk_bytes, 1) != 1), 0))
   do { fatal(control, 291, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to read chunk_bytes size in runzip_chunk\n"); return -1; } while (0);
  if (__builtin_expect(!!(chunk_bytes < 1 || chunk_bytes > 8), 0))
   do { failure(control, 293, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "chunk_bytes %d is invalid in runzip_chunk\n", chunk_bytes); return -1; } while (0);
 }
 if (!tally && expected_size)
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 296, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Expected size: %lld\n", expected_size); } while (0); } while (0);
 do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 297, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Chunk byte width: %d\n", chunk_bytes); } while (0); } while (0);

 ofs = seekcur_fdin(control);
 if (__builtin_expect(!!(ofs == -1), 0))
  do { fatal(control, 301, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to seek input file in runzip_fd\n"); return -1; } while (0);

 if (fstat(fd_in, &st) || st.st_size - ofs == 0)
  return 0;

 ss = open_stream_in(control, fd_in, 2, chunk_bytes);
 if (__builtin_expect(!!(!ss), 0))
  do { failure(control, 308, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to open_stream_in in runzip_chunk\n"); return -1; } while (0);
 ...
 while ((len = read_header(control, ss, &head)) || head) {
  ...
  if (expected_size) {
   ...
   if (p / 10 != l / 10) {
    ...
    do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 344, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "%3d%%  %9.2f / %9.2f %s\r", p, prog_done, prog_tsize, suffix[divisor_index]); } while (0); } while (0);
 ...
 if (!(control->flags & (1 << 17))) {
  ...
  if (__builtin_expect(!!(good_cksum != cksum), 0)) {
   close_stream_in(control, ss);
   do { failure(control, 358, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Bad checksum: 0x%08x - expected: 0x%08x\n", cksum, good_cksum); return -1; } while (0);
  }
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 360, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Checksum for block: 0x%08x\n", cksum); } while (0); } while (0);
 }

 if (__builtin_expect(!!(close_stream_in(control, ss)), 0))
  fatal(control, 364, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to close stream!\n");
...
i64 runzip_fd(rzip_control *control, int fd_in, int fd_out, int fd_hist, i64 expected_size)
{
 ...
 do {
  ...
  if (u < 1) {
   if (u < 0 || total < expected_size) {
    do { print_err(control, 387, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to runzip_chunk in runzip_fd\n"); } while (0);
    return -1;
   }
  }
  total += u;
  if ((control->flags & (1 << 21))) {
   if (__builtin_expect(!!(!flush_tmpoutbuf(control)), 0)) {
    do { print_err(control, 394, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to flush_tmpoutbuf in runzip_fd\n"); } while (0);
    return -1;
   }
  } else if ((control->flags & (1 << 13))) {
   if (__builtin_expect(!!(!dump_tmpoutfile(control, fd_out)), 0)) {
    do { print_err(control, 399, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to dump_tmpoutfile in runzip_fd\n"); } while (0);
    return -1;
   }
  }
  if ((control->flags & (1 << 22)))
   clear_tmpinbuf(control);
  else if ((control->flags & (1 << 12)) && !(control->flags & (1 << 4))) {
   if (__builtin_expect(!!(!clear_tmpinfile(control)), 0)) {
    do { print_err(control, 407, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to clear_tmpinfile in runzip_fd\n"); } while (0);
    return -1;
   }
  }
 ...
 if (!(control->flags & (1 << 23))) {
  ...
  if (!tdiff)
   tdiff = 1;
  do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 419, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "\nAverage DeCompression Speed: %6.3fMB/s\n", (total / 1024 / 1024) / tdiff); } while (0); } while (0);
 ...
 if (!(!((control->flags & (1 << 16))) && !((control->flags & (1 << 17))))) {
  ...
  if ((control->flags & (1 << 17))) {
   ...
   if (__builtin_expect(!!(fdinend == -1), 0))
    do { failure(control, 430, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to seekto_fdinend in rzip_fd\n"); return -1; } while (0);
   if (__builtin_expect(!!(seekto_fdin(control, fdinend - 16) == -1), 0))
    do { failure(control, 432, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to seekto_fdin in rzip_fd\n"); return -1; } while (0);

   if (__builtin_expect(!!(read_1g(control, fd_in, md5_stored, 16) != 16), 0))
    do { fatal(control, 435, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to read md5 data in runzip_fd\n"); return -1; } while (0);
   if ((control->flags & (1 << 23)))
    if (__builtin_expect(!!(!lrz_decrypt(control, md5_stored, 16, control->salt_pass)), 0))
     return -1;
   for (i = 0; i < 16; i++)
    if (md5_stored[i] != control->md5_resblock[i]) {
     do { do { print_stuff(control, 1, 441, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "MD5 CHECK FAILED.\nStored:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 443, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "%02x", md5_stored[j] & 0xFF); } while (0); } while (0);
     do { do { print_stuff(control, 1, 444, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "\nOutput file:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 446, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "%02x", control->md5_resblock[j] & 0xFF); } while (0); } while (0);
     do { failure(control, 447, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "\n"); return -1; } while (0);
    }
  }

  if ((control->flags & (1 << 16)) || (control->flags & (1 << 11))) {
   do { do { print_stuff(control, 1, 452, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "MD5: "); } while (0); } while (0);
   for (i = 0; i < 16; i++)
    do { do { print_stuff(control, 1, 454, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "%02x", control->md5_resblock[i] & 0xFF); } while (0); } while (0);
   do { do { print_stuff(control, 1, 455, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "\n"); } while (0); } while (0);
  }
  ...
  if ((control->flags & (1 << 18))) {
   ...
   memcpy(md5_stored, control->md5_resblock, 16);
   if (__builtin_expect(!!(seekto_fdhist(control, 0) == -1), 0))
    do { fatal(control, 466, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to seekto_fdhist in runzip_fd\n"); return -1; } while (0);
   if (__builtin_expect(!!((md5_fstream = fdopen(fd_hist, "r")) == ((void*)0)), 0))
    do { fatal(control, 468, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to fdopen fd_hist in runzip_fd\n"); return -1; } while (0);
   if (__builtin_expect(!!(md5_stream(md5_fstream, control->md5_resblock)), 0))
    do { fatal(control, 470, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Failed to md5_stream in runzip_fd\n"); return -1; } while (0);

   for (i = 0; i < 16; i++)
    if (md5_stored[i] != control->md5_resblock[i]) {
     do { do { print_stuff(control, 1, 474, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "MD5 CHECK FAILED.\nStored:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 476, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "%02x", md5_stored[j] & 0xFF); } while (0); } while (0);
     do { do { print_stuff(control, 1, 477, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "\nOutput file:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 479, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "%02x", control->md5_resblock[j] & 0xFF); } while (0); } while (0);
     do { failure(control, 480, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "\n"); return -1; } while (0);
    }
   do { do { print_stuff(control, 1, 482, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "MD5 integrity of written file matches archive\n"); } while (0); } while (0);
   if (!(control->flags & (1 << 17)))
    do { do { print_stuff(control, 1, 486, "/benchmark/RUNDIR-lrzip-ed51e14/BUILD/runzip.c", __func__, "Note this lrzip archive did not have a stored md5 value.\n" "The archive decompression was validated with crc32 and the md5 hash was " "calculated on decompression\n"); } while (0); } while (0);
