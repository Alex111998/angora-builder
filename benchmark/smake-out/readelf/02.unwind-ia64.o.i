# 1 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 340 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c" 2
# 23 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c"
# 1 "./config.h" 1
# 24 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c" 2
# 1 "./unwind-ia64.h" 1
# 22 "./unwind-ia64.h"
...
# 285 "./../include/elf/ia64.h"
# 1 "./../include/elf/reloc-macros.h" 1
# 286 "./../include/elf/ia64.h" 2

enum elf_ia64_reloc_type {
  R_IA64_NONE = 0x00,
...
# 25 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c" 2
...
# 26 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c" 2
...
# 27 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c" 2
...
# 36 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c" 2
...
# 174 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c"
typedef bfd_vma unw_word;
# 544 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/binutils/unwind-ia64.c"
static unw_word
...
unw_decode_x1 (const unsigned char *dp, unsigned int code __attribute__ ((__unused__)),
        void *arg __attribute__ ((__unused__)))
{
  ...
  abreg = (byte1 & 0x7f);
  if (byte1 & 0x80)
    do { char regname[20]; unw_print_abreg (regname, abreg); printf ("\t%s:spill_sprel(reg=%s,t=%lu,spoff=0x%lx)\n", "X1", regname, (unsigned long) t, 4*(unsigned long)off); } while (0);
  else
    do { char regname[20]; unw_print_abreg (regname, abreg); printf ("\t%s:spill_psprel(reg=%s,t=%lu,pspoff=0x10-0x%lx)\n", "X1", regname, (unsigned long) t, 4*(unsigned long)off); } while (0);
  return dp;
}

static const unsigned char *
unw_decode_x2 (const unsigned char *dp, unsigned int code __attribute__ ((__unused__)),
        void *arg __attribute__ ((__unused__)))
{
  ...
  if ((byte1 & 0x80) == 0 && ytreg == 0)
    do { char regname[20]; unw_print_abreg (regname, abreg); printf ("\t%s:restore(t=%lu,reg=%s)\n", "X2", (unsigned long) t, regname); } while (0);
  else
    do { char abregname[20], tregname[20]; unw_print_abreg (abregname, abreg); unw_print_xyreg (tregname, x, ytreg); printf ("\t%s:spill_reg(t=%lu,reg=%s,treg=%s)\n", "X2", (unsigned long) t, abregname, tregname); } while (0);
  return dp;
}

static const unsigned char *
unw_decode_x3 (const unsigned char *dp, unsigned int code __attribute__ ((__unused__)),
        void *arg __attribute__ ((__unused__)))
{
  ...
  if (byte1 & 0x80)
    do { char regname[20]; unw_print_abreg (regname, abreg); printf ("\t%s:spill_sprel_p(qp=p%u,t=%lu,reg=%s,spoff=0x%lx)\n", "X3", qp, (unsigned long) t, regname, 4 * (unsigned long)off); } while (0);
  else
    do { char regname[20]; unw_print_abreg (regname, abreg); printf ("\t%s:spill_psprel_p(qp=p%u,t=%lu,reg=%s,pspoff=0x10-0x%lx)\n", "X3", qp, (unsigned long) t, regname, 4*(unsigned long)off); } while (0);
  return dp;
}

static const unsigned char *
unw_decode_x4 (const unsigned char *dp, unsigned int code __attribute__ ((__unused__)),
        void *arg __attribute__ ((__unused__)))
{
  ...
  if ((byte2 & 0x80) == 0 && byte3 == 0)
    do { char regname[20]; unw_print_abreg (regname, abreg); printf ("\t%s:restore_p(qp=p%u,t=%lu,reg=%s)\n", "X4", qp, (unsigned long) t, regname); } while (0);
  else
    do { char regname[20], tregname[20]; unw_print_abreg (regname, abreg); unw_print_xyreg (tregname, x, ytreg); printf ("\t%s:spill_reg_p(qp=p%u,t=%lu,reg=%s,treg=%s)\n", "X4", qp, (unsigned long) t, regname, tregname); } while (0);
  return dp;
}

static const unsigned char *
unw_decode_r1 (const unsigned char *dp, unsigned int code, void *arg,
        const unsigned char * end __attribute__ ((__unused__)))
{
  int body = (code & 0x20) != 0;
  unw_word rlen;

  rlen = (code & 0x1f);
  do { unw_rlen = rlen; *(int *)arg = body; printf ("    %s:%s(rlen=%lu)\n", "R1", body ? "body" : "prologue", (unsigned long) rlen); } while (0);
  return dp;
}
        ...
        const unsigned char * end __attribute__ ((__unused__)))
{
  ...
  rlen = unw_decode_uleb128 (& dp);
  do { unw_rlen = rlen; *(int *)arg = ((code & 0x3) == 1); printf ("    %s:%s(rlen=%lu)\n", "R3", ((code & 0x3) == 1) ? "body" : "prologue", (unsigned long) rlen); } while (0);
  return dp;
}
        ...
        const unsigned char * end __attribute__ ((__unused__)))
{
  ...
  do { char brstr[20]; unw_print_brmask (brstr, brmask); printf ("\t%s:br_mem(brmask=[%s])\n", "P1", brstr); } while (0);
  return dp;
}
    ...
    const unsigned char * end)
{
  if ((code & 0x10) == 0)
    {
      ...
      do { char brstr[20]; unw_print_brmask (brstr, ((code & 0xf) << 1) | ((byte1 >> 7) & 1)); printf ("\t%s:br_gr(brmask=[%s],gr=r%u)\n", "P2", brstr, (byte1 & 0x7f)); } while (0);
 ...
 default:
   ...
   break;
    ...
    do { static const char *spill_type = "-frb"; unsigned const char *imaskp = dp; unsigned char mask = 0; bfd_vma insn = 0; if ((dp + (unw_rlen / 4)) > end) { printf ("\nERROR: unwind length too long (0x%lx > 0x%lx)\n\n", (long) (unw_rlen / 4), (long)(end - dp)); break; } printf ("\t%s:spill_mask(imask=[", "P4"); for (insn = 0; insn < unw_rlen; ++insn) { if ((insn % 4) == 0) mask = *imaskp++; if (insn > 0 && (insn % 3) == 0) putchar (','); putchar (spill_type[(mask >> (2 * (3 - (insn & 0x3)))) & 0x3]); } printf ("])\n"); dp = imaskp; } while (0);
  else if ((code & 0x7) == 1)
    {
      ...
      do { char frstr[200], grstr[20]; unw_print_grmask (grstr, grmask); unw_print_frmask (frstr, frmask); printf ("\t%s:frgr_mem(grmask=[%s],frmask=[%s])\n", "P5", grstr, frstr); } while (0);
    }
  else
    printf ("Unknown code 0x%02x\n", code);
        ...
        const unsigned char * end __attribute__ ((__unused__)))
{
  ...
  if (gregs)
    do { char grstr[200]; unw_print_grmask (grstr, mask); printf ("\t%s:gr_mem(grmask=[%s])\n", "P6", grstr); } while (0);
  else
    do { char frstr[200]; unw_print_frmask (frstr, mask); printf ("\t%s:fr_mem(frmask=[%s])\n", "P6", frstr); } while (0);
  return dp;
}
     ...
     const unsigned char * end __attribute__ ((__unused__)))
{
 ...
 case 0:
   ...
   printf ("\t%s:mem_stack_f(t=%lu,size=%lu)\n", "P7", (unsigned long) t, 16*(unsigned long)size);
   break;
 ...
 case 1:
   ...
   break;
 case 2:
   printf ("\t%s:spill_base(pspoff=0x10-0x%lx)\n", "P7", 4*(unsigned long)t);
   break;
 case 3:
   printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P7", "psp", 4*(unsigned long)t);
   break;
 case 4:
   printf ("\t%s:%s_when(t=%lu)\n", "P7", "rp", (unsigned long) t);
   break;
 case 5:
   printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P7", "rp", 4*(unsigned long)t);
   break;
 case 6:
   printf ("\t%s:%s_when(t=%lu)\n", "P7", "pfs", (unsigned long) t);
   break;
 case 7:
   printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P7", "pfs", 4*(unsigned long)t);
   break;
 case 8:
   printf ("\t%s:%s_when(t=%lu)\n", "P7", "pr", (unsigned long) t);
   break;
 case 9:
   printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P7", "pr", 4*(unsigned long)t);
   break;
 case 10:
   printf ("\t%s:%s_when(t=%lu)\n", "P7", "lc", (unsigned long) t);
   break;
 case 11:
   printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P7", "lc", 4*(unsigned long)t);
   break;
 case 12:
   printf ("\t%s:%s_when(t=%lu)\n", "P7", "unat", (unsigned long) t);
   break;
 case 13:
   printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P7", "unat", 4*(unsigned long)t);
   break;
 case 14:
   printf ("\t%s:%s_when(t=%lu)\n", "P7", "fpsr", (unsigned long) t);
   break;
 case 15:
   printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P7", "fpsr", 4*(unsigned long)t);
   break;
 default:
   printf ("Unknown code 0x%02x\n", r);
   break;
 ...
 case 0x0:
   {
     ...
     switch (r)
       {
       case 1:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "rp", 4*(unsigned long)t);
  break;
       case 2:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "pfs", 4*(unsigned long)t);
  break;
       case 3:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "pr", 4*(unsigned long)t);
  break;
       case 4:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "lc", 4*(unsigned long)t);
  break;
       case 5:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "unat", 4*(unsigned long)t);
  break;
       case 6:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "fpsr", 4*(unsigned long)t);
  break;
       case 7:
  printf ("\t%s:%s_when(t=%lu)\n", "P8", "bsp", (unsigned long) t);
  break;
       case 8:
  printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P8", "bsp", 4*(unsigned long)t);
  break;
       case 9:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "bsp", 4*(unsigned long)t);
  break;
       case 10:
  printf ("\t%s:%s_when(t=%lu)\n", "P8", "bspstore", (unsigned long) t);
  break;
       case 11:
  printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P8", "bspstore", 4*(unsigned long)t);
  break;
       case 12:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "bspstore", 4*(unsigned long)t);
  break;
       case 13:
  printf ("\t%s:%s_when(t=%lu)\n", "P8", "rnat", (unsigned long) t);
  break;
       case 14:
  printf ("\t%s:%s_psprel(pspoff=0x10-0x%lx)\n", "P8", "rnat", 4*(unsigned long)t);
  break;
       case 15:
  printf ("\t%s:%s_sprel(spoff=0x%lx)\n", "P8", "rnat", 4*(unsigned long)t);
  break;
 ...
 case 0x1:
   byte1 = *dp++;
   byte2 = *dp++;
   do { char grstr[20]; unw_print_grmask (grstr, (byte1 & 0xf)); printf ("\t%s:gr_gr(grmask=[%s],r%u)\n", "P9", grstr, (byte2 & 0x7f)); } while (0);
   break;

 case 0xf:
   byte1 = *dp++;
   byte2 = *dp++;
   do { static const char * const abiname[] = { "@svr4", "@hpux", "@nt" }; char buf[20]; const char *abistr = buf; if (byte1 < 3) abistr = abiname[byte1]; else sprintf (buf, "0x%x", byte1); printf ("\t%s:unwabi(abi=%s,context=0x%02x)\n", "P10", abistr, byte2); } while (0);
   break;
        ...
        const unsigned char * end __attribute__ ((__unused__)))
{
  ...
  t = unw_decode_uleb128 (& dp);
  printf ("\t%s:epilogue(t=%lu,ecount=%lu)\n", "B2", (unsigned long) t, (unsigned long) (code & 0x1f));
  return dp;
}
  ...
  if ((code & 0x10) == 0)
    {
      ...
      printf ("\t%s:epilogue(t=%lu,ecount=%lu)\n", "B3", (unsigned long) t, (unsigned long) ecount);
    }
  else if ((code & 0x07) == 0)
    {
      label = unw_decode_uleb128 (&dp);
      if ((code & 0x08) != 0)
 printf ("\t%s:copy_state(label=%lu)\n", "B4", (unsigned long) label);
      else
 printf ("\t%s:label_state(label=%lu)\n", "B4", (unsigned long) label);
    }
  else
