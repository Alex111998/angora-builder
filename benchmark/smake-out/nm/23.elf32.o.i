# 1 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/bfd/elf32.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/bfd/elf32.c" 2
# 23 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/bfd/elf32.c"
# 1 "./elfcode.h" 1
# 68 "./elfcode.h"
# 1 "./sysdep.h" 1
# 29 "./sysdep.h"
...
bfd_elf32_object_p (bfd *abfd)
{
      ...
      if (i_ehdrp->e_shstrndx >= (((abfd) -> tdata.elf_obj_data) -> num_elf_sections))
 {
   ...
   _bfd_error_handler
     (dgettext ("bfd", "warning: %B has a corrupt string table index - ignoring"),
      abfd);
 }
    }
...
# 1148 "./elfcode.h"
   ...
   && verhdr->sh_size / sizeof (Elf_External_Versym) != symcount)
 {
   _bfd_error_handler
     ...
     (dgettext ("bfd", "%B: version count (%ld) does not match symbol count (%ld)"),
      abfd,
        ...
        bfd_boolean dynamic)
{
  ...
  do { if (!(entsize == sizeof (Elf32_External_Rel) || entsize == sizeof (Elf32_External_Rela))) bfd_assert("./elfcode.h",1415); } while (0);
      ...
      else if (((rela.r_info) >> 8) > symcount)
 {
   _bfd_error_handler
     ...
     (dgettext ("bfd", "%B(%A): relocation %d has invalid symbol index %ld"),
         ...
         bfd_boolean dynamic)
{
 ...
 return 0;
      do { if (!((rel_hdr && asect->rel_filepos == rel_hdr->sh_offset) || (rel_hdr2 && asect->rel_filepos == rel_hdr2->sh_offset))) bfd_assert("./elfcode.h",1516); } while (0);
...
# 24 "/benchmark/RUNDIR-binutils-2.29/binutils-2.29/bfd/elf32.c" 2
