# 1 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 343 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
# 21 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c"
# 1 "./config.h" 1
# 22 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
# 25 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
# 27 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
# 30 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
# 33 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
# 41 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
# 44 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
# 1 "./runzip.h" 1
# 25 "./runzip.h"
i64 runzip_fd(rzip_control *control, int fd_in, int fd_out, int fd_hist, i64 expected_size);
# 45 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
# 1 "./stream.h" 1
# 26 "./stream.h"
...
# 46 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
# 1 "./util.h" 1
# 23 "./util.h"
...
# 47 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
# 1 "./lrzip_core.h" 1
# 24 "./lrzip_core.h"
...
extern void zpaq_decompress(unsigned char *s_buf, i64 *d_len, unsigned char *c_buf, i64 c_len,
       FILE *msgout, _Bool progress, long thread);
# 48 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
# 50 "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c" 2
...
static inline unsigned char read_u8(rzip_control *control, void *ss, int stream, _Bool *err)
{
  ...
  *err = 1;
  do { fatal(control, 57, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Stream read u8 failed\n"); return 0; } while (0);
 }
 return b;
}
...
static inline u32 read_u32(rzip_control *control, void *ss, int stream, _Bool *err)
{
  ...
  *err = 1;
  do { fatal(control, 68, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Stream read u32 failed\n"); return 0; } while (0);
 }
...
static inline i64 read_vchars(rzip_control *control, void *ss, int stream, int length)
{
 ...
 if (__builtin_expect(!!(read_stream(control, ss, stream, (unsigned char *)&s, length) != length), 0))
  do { fatal(control, 80, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Stream read of %d bytes failed\n", length); return -1; } while (0);
...
static i64 seekto_fdhist(rzip_control *control, i64 pos)
{
 ...
 if (__builtin_expect(!!(control->hist_ofs < 0 || control->hist_ofs > control->out_maxlen), 0)) {
  do { print_err(control, 100, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Trying to seek outside tmpoutbuf to %lld in seekto_fdhist\n", control->hist_ofs); } while (0);
  return -1;
 }
 return pos;
}
...
static i64 seekto_fdin(rzip_control *control, i64 pos)
{
 ...
 if (__builtin_expect(!!(pos > control->in_len || pos < 0), 0)) {
  do { print_err(control, 118, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Trying to seek outside tmpinbuf to %lld in seekto_fdin\n", pos); } while (0);
  return -1;
 }
 control->in_ofs = pos;
 return 0;
...
static i64 seekto_fdinend(rzip_control *control)
{
 ...
 while ((tmpchar = getchar()) != (-1)) {
  ...
  if (__builtin_expect(!!(control->in_len > control->in_maxlen), 0))
   do { failure(control, 134, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Trying to read greater than max_len\n"); return -1; } while (0);
 }
...
static i64 unzip_literal(rzip_control *control, void *ss, i64 len, unsigned int *cksum)
{
 ...
 if (__builtin_expect(!!(len < 0), 0))
  do { failure(control, 156, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "len %lld is negative in unzip_literal!\n",len); return -1; } while (0);

 buf = (unsigned char *)malloc(len);
 if (__builtin_expect(!!(!buf), 0))
  do { fatal(control, 160, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to malloc literal buffer of size %lld\n", len); return -1; } while (0);

 stream_read = read_stream(control, ss, 1, buf, len);
 if (__builtin_expect(!!(stream_read == -1), 0)) {
  do { free((buf)); (buf) = ((void*)0); } while (0);
  do { fatal(control, 165, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to read_stream in unzip_literal\n"); return -1; } while (0);
 }

 if (__builtin_expect(!!(write_1g(control, buf, (size_t)stream_read) != (ssize_t)stream_read), 0)) {
  do { free((buf)); (buf) = ((void*)0); } while (0);
  do { fatal(control, 170, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to write literal buffer of size %lld\n", stream_read); return -1; } while (0);
 }
...
static i64 read_fdhist(rzip_control *control, void *buf, i64 len)
{
 ...
 if (__builtin_expect(!!(len + control->hist_ofs > control->out_maxlen), 0)) {
  do { print_err(control, 187, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Trying to read beyond end of tmpoutbuf in read_fdhist\n"); } while (0);
  return -1;
 }
...
static i64 unzip_match(rzip_control *control, void *ss, i64 len, unsigned int *cksum, int chunk_bytes)
{
 ...
 if (__builtin_expect(!!(len < 0), 0))
  do { failure(control, 200, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "len %lld is negative in unzip_match!\n",len); return -1; } while (0);

 total = 0;
 cur_pos = seekcur_fdout(control);
 if (__builtin_expect(!!(cur_pos == -1), 0))
  do { fatal(control, 205, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Seek failed on out file in unzip_match.\n"); return -1; } while (0);
 ...
 if (__builtin_expect(!!(seekto_fdhist(control, cur_pos - offset) == -1), 0))
  do { fatal(control, 213, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Seek failed by %d from %d on history file in unzip_match\n", offset, cur_pos); return -1; } while (0);


 buf = (unsigned char *)malloc(len);
 if (__builtin_expect(!!(!buf), 0))
  do { fatal(control, 217, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to malloc match buffer of size %lld\n", len); return -1; } while (0);
 off_buf = buf;
 ...
 while (len) {
  ...
  if (__builtin_expect(!!(read_fdhist(control, off_buf, (size_t)n) != (ssize_t)n), 0)) {
   do { free((buf)); (buf) = ((void*)0); } while (0);
   do { fatal(control, 225, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to read %d bytes in unzip_match\n", n); return -1; } while (0);
  }
  if (__builtin_expect(!!(write_1g(control, off_buf, (size_t)n) != (ssize_t)n), 0)) {
   do { free((buf)); (buf) = ((void*)0); } while (0);
   do { fatal(control, 229, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to write %d bytes in unzip_match\n", n); return -1; } while (0);
  }
...
static i64 runzip_chunk(rzip_control *control, int fd_in, i64 expected_size, i64 tally)
{
 ...
 if (control->major_version == 0 && control->minor_version < 4)
  ...
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 286, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Reading chunk_bytes at %lld\n", get_readseek(control, fd_in)); } while (0); } while (0);

  if (__builtin_expect(!!(read_1g(control, fd_in, &chunk_bytes, 1) != 1), 0))
   do { fatal(control, 289, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to read chunk_bytes size in runzip_chunk\n"); return -1; } while (0);
  if (__builtin_expect(!!(chunk_bytes < 1 || chunk_bytes > 8), 0))
   do { failure(control, 291, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "chunk_bytes %d is invalid in runzip_chunk\n", chunk_bytes); return -1; } while (0);
 }
 if (!tally && expected_size)
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 294, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Expected size: %lld\n", expected_size); } while (0); } while (0);
 do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 295, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Chunk byte width: %d\n", chunk_bytes); } while (0); } while (0);

 ofs = seekcur_fdin(control);
 if (__builtin_expect(!!(ofs == -1), 0))
  do { fatal(control, 299, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to seek input file in runzip_fd\n"); return -1; } while (0);
 ...
 if (__builtin_expect(!!(!ss), 0))
  do { failure(control, 306, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to open_stream_in in runzip_chunk\n"); return -1; } while (0);
 ...
 while ((len = read_header(control, ss, &head)) || head) {
  ...
  if (expected_size) {
   ...
   if (p / 10 != l / 10) {
    ...
    do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 342, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "%3d%%  %9.2f / %9.2f %s\r", p, prog_done, prog_tsize, suffix[divisor_index]); } while (0); } while (0);
 ...
 if (!(control->flags & (1 << 17))) {
  ...
  if (__builtin_expect(!!(good_cksum != cksum), 0)) {
   close_stream_in(control, ss);
   do { failure(control, 356, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Bad checksum: 0x%08x - expected: 0x%08x\n", cksum, good_cksum); return -1; } while (0);
  }
  do { if ((control->flags & (1 << 11))) do { print_stuff(control, 4, 358, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Checksum for block: 0x%08x\n", cksum); } while (0); } while (0);
 }

 if (__builtin_expect(!!(close_stream_in(control, ss)), 0))
  fatal(control, 362, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to close stream!\n");
...
i64 runzip_fd(rzip_control *control, int fd_in, int fd_out, int fd_hist, i64 expected_size)
{
 ...
 do {
  ...
  if (__builtin_expect(!!(u == -1), 0)) {
   do { print_err(control, 384, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to runzip_chunk in runzip_fd\n"); } while (0);
   return -1;
  }
  total += u;
  if ((control->flags & (1 << 21))) {
   if (__builtin_expect(!!(!flush_tmpoutbuf(control)), 0)) {
    do { print_err(control, 390, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to flush_tmpoutbuf in runzip_fd\n"); } while (0);
    return -1;
   }
  } else if ((control->flags & (1 << 13))) {
   if (__builtin_expect(!!(!dump_tmpoutfile(control, fd_out)), 0)) {
    do { print_err(control, 395, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to dump_tmpoutfile in runzip_fd\n"); } while (0);
    return -1;
   }
  }
  if ((control->flags & (1 << 22)))
   ...
   if (__builtin_expect(!!(!clear_tmpinfile(control)), 0)) {
    do { print_err(control, 403, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to clear_tmpinfile in runzip_fd\n"); } while (0);
    return -1;
   }
  }
 ...
 if (!(control->flags & (1 << 23))) {
  ...
  if (!tdiff)
   tdiff = 1;
  do { if ((control->flags & (1 << 0))) do { print_stuff(control, 2, 415, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "\nAverage DeCompression Speed: %6.3fMB/s\n", (total / 1024 / 1024) / tdiff); } while (0); } while (0);
 ...
 if (!(!((control->flags & (1 << 16))) && !((control->flags & (1 << 17))))) {
  ...
  if ((control->flags & (1 << 17))) {
   ...
   if (__builtin_expect(!!(fdinend == -1), 0))
    do { failure(control, 426, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to seekto_fdinend in rzip_fd\n"); return -1; } while (0);
   if (__builtin_expect(!!(seekto_fdin(control, fdinend - 16) == -1), 0))
    do { failure(control, 428, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to seekto_fdin in rzip_fd\n"); return -1; } while (0);

   if (__builtin_expect(!!(read_1g(control, fd_in, md5_stored, 16) != 16), 0))
    do { fatal(control, 431, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to read md5 data in runzip_fd\n"); return -1; } while (0);
   if ((control->flags & (1 << 23)))
    if (__builtin_expect(!!(!lrz_decrypt(control, md5_stored, 16, control->salt_pass)), 0))
     return -1;
   for (i = 0; i < 16; i++)
    if (md5_stored[i] != control->md5_resblock[i]) {
     do { do { print_stuff(control, 1, 437, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "MD5 CHECK FAILED.\nStored:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 439, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "%02x", md5_stored[j] & 0xFF); } while (0); } while (0);
     do { do { print_stuff(control, 1, 440, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "\nOutput file:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 442, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "%02x", control->md5_resblock[j] & 0xFF); } while (0); } while (0);
     do { failure(control, 443, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "\n"); return -1; } while (0);
    }
  }

  if ((control->flags & (1 << 16)) || (control->flags & (1 << 11))) {
   do { do { print_stuff(control, 1, 448, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "MD5: "); } while (0); } while (0);
   for (i = 0; i < 16; i++)
    do { do { print_stuff(control, 1, 450, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "%02x", control->md5_resblock[i] & 0xFF); } while (0); } while (0);
   do { do { print_stuff(control, 1, 451, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "\n"); } while (0); } while (0);
  }
  ...
  if ((control->flags & (1 << 18))) {
   ...
   if (__builtin_expect(!!(seekto_fdhist(control, 0) == -1), 0))
    do { fatal(control, 462, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to seekto_fdhist in runzip_fd\n"); return -1; } while (0);
   if (__builtin_expect(!!((md5_fstream = fdopen(fd_hist, "r")) == ((void*)0)), 0))
    do { fatal(control, 464, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to fdopen fd_hist in runzip_fd\n"); return -1; } while (0);
   if (__builtin_expect(!!(md5_stream(md5_fstream, control->md5_resblock)), 0))
    do { fatal(control, 466, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Failed to md5_stream in runzip_fd\n"); return -1; } while (0);

   for (i = 0; i < 16; i++)
    if (md5_stored[i] != control->md5_resblock[i]) {
     do { do { print_stuff(control, 1, 470, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "MD5 CHECK FAILED.\nStored:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 472, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "%02x", md5_stored[j] & 0xFF); } while (0); } while (0);
     do { do { print_stuff(control, 1, 473, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "\nOutput file:"); } while (0); } while (0);
     for (j = 0; j < 16; j++)
      do { do { print_stuff(control, 1, 475, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "%02x", control->md5_resblock[j] & 0xFF); } while (0); } while (0);
     do { failure(control, 476, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "\n"); return -1; } while (0);
    }
   do { do { print_stuff(control, 1, 478, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "MD5 integrity of written file matches archive\n"); } while (0); } while (0);
   if (!(control->flags & (1 << 17)))
    do { do { print_stuff(control, 1, 482, "/benchmark/RUNDIR-lrzip-9de7ccb/BUILD/runzip.c", __func__, "Note this lrzip archive did not have a stored md5 value.\n" "The archive decompression was validated with crc32 and the md5 hash was " "calculated on decompression\n"); } while (0); } while (0);
