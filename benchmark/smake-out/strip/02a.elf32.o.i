# 1 "/benchmark/RUNDIR-binutils-2.27/binutils-2.27/bfd/elf32.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/benchmark/RUNDIR-binutils-2.27/binutils-2.27/bfd/elf32.c" 2
# 23 "/benchmark/RUNDIR-binutils-2.27/binutils-2.27/bfd/elf32.c"
# 1 "./elfcode.h" 1
# 68 "./elfcode.h"
# 1 "./sysdep.h" 1
# 29 "./sysdep.h"
...
bfd_elf32_object_p (bfd *abfd)
{
      ...
      if (i_ehdrp->e_shstrndx >= (((abfd) -> tdata.elf_obj_data) -> num_elf_sections))
 {
   ...
   _bfd_error_handler (dgettext ("bfd", "warning: %s has a corrupt string table index - ignoring"), abfd->filename);
 }
    }
...
bfd_elf32_write_relocs (bfd *abfd, asection *sec, void *data)
{
  ...
  if (rela_hdr->sh_type == 4)
    ...
    _bfd_abort ("./elfcode.h", 912, __PRETTY_FUNCTION__);
...
# 1145 "./elfcode.h"
 ...
 {
   (*_bfd_error_handler)
     (dgettext ("bfd", "%s: version count (%ld) does not match symbol count (%ld)"),
      abfd->filename,
      ...
      for (isym = isymbuf + 1, sym = symbase; isym < isymend; isym++, sym++)
 {
       ...
       if ((abfd->flags & 0x10000) != 0)
  {
    ...
    if (xc == ((void*)0))
      {
        flagword flags = (0x001 | 0x1000 | 0x200000
     | 0x8000);
        ...
        bfd_boolean dynamic)
{
  ...
  do { if (!(entsize == sizeof (Elf32_External_Rel) || entsize == sizeof (Elf32_External_Rela))) bfd_assert("./elfcode.h",1411); } while (0);
 ...
 {
   (*_bfd_error_handler)
     (dgettext ("bfd", "%s(%s): relocation %d has invalid symbol index %ld"),
      abfd->filename, asect->name, i, ((rela.r_info) >> 8));
   relent->sym_ptr_ptr = (&_bfd_std_section[2])->symbol_ptr_ptr;
 }
      else
 {
         ...
         bfd_boolean dynamic)
{
 ...
 return 0;
      do { if (!((rel_hdr && asect->rel_filepos == rel_hdr->sh_offset) || (rel_hdr2 && asect->rel_filepos == rel_hdr2->sh_offset))) bfd_assert("./elfcode.h",1511); } while (0);
...
bfd_elf32_core_file_p (bfd *abfd)
{
 ...
 if ((bfd_size_type) statbuf.st_size < high)
   {
     (*_bfd_error_handler)
       (dgettext ("bfd", "Warning: %B is truncated: expected core file " "size >= %lu, found: %lu."),
...
# 24 "/benchmark/RUNDIR-binutils-2.27/binutils-2.27/bfd/elf32.c" 2
